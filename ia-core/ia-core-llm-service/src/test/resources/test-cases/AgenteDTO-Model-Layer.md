# Caso de Teste: AgenteDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `AgenteDTO`. Cadastro, consulta, atualização e associação de agentes LLM com ferramentas e skills.
...
## Objetivo
Documentar e validar a camada modelo/dto de `AgenteDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgenteDTO` no domínio Agente LLM.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `AgenteDTO`.
...
**When**: A camada modelo/dto valida ou transforma `AgenteDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgenteDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
