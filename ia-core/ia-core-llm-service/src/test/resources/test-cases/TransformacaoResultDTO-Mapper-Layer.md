# Caso de Teste: TransformacaoResultDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `TransformacaoResultDTO`. Resultado agregado de transformação de ontologia com axiomas e inconsistências.
...
## Objetivo
Documentar e validar a camada mapper de `TransformacaoResultDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TransformacaoResultDTO` no domínio Transformação OWL.
2. Quando a camada mapper recebe ou produz dados compatíveis com `TransformacaoResultDTO`.
...
**When**: A camada mapper valida ou transforma `TransformacaoResultDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TransformacaoResultDTO - Mapper` |
| Camada | Mapper |
