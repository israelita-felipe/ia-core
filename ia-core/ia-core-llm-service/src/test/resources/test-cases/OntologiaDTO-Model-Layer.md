# Caso de Teste: OntologiaDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `OntologiaDTO`. Representação, validação, exportação e manutenção de ontologias OWL.

## Classe Testada
`com.ia.core.llm.model.ontologia.Ontologia`
...
## Objetivo
Documentar e validar a camada modelo/dto de `OntologiaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `OntologiaDTO` no domínio Ontologia OWL.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `OntologiaDTO`.
...
**When**: A camada modelo/dto valida ou transforma `OntologiaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OntologiaDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
