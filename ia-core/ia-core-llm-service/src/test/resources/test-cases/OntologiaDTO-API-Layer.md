# Caso de Teste: OntologiaDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `OntologiaDTO`. Representação, validação, exportação e manutenção de ontologias OWL.

## Classe Testada
`com.ia.core.llm.rest.web.OntologiasController`
...
## Objetivo
Documentar e validar a camada api/rest de `OntologiaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `OntologiaDTO` no domínio Ontologia OWL.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `OntologiaDTO`.
...
**When**: A camada api/rest valida ou transforma `OntologiaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OntologiaDTO - API/REST` |
| Camada | API/REST |
