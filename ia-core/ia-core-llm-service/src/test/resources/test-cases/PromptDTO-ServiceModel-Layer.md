# Caso de Teste: PromptDTO - Service Model

## Descrição
Caso de teste para a camada service model do DTO `PromptDTO`. Contrato de prompt de catálogo vinculado a template e finalidade LLM.

## Classe Testada
`com.ia.core.llm.service.model...PromptDTO`
...
## Objetivo
Documentar e validar a camada service model de `PromptDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `PromptDTO` no domínio Prompt.
2. Quando a camada service model recebe ou produz dados compatíveis com `PromptDTO`.
...
**When**: A camada service model valida ou transforma `PromptDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PromptDTO - Service Model` |
