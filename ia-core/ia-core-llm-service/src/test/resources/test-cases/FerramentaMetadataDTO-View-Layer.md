# Caso de Teste: FerramentaMetadataDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `FerramentaMetadataDTO`. Metadados resumidos de ferramentas disponíveis para seleção pelo agente/view.

## Classe Testada
`com.ia.core.llm.view.ferramenta.FerramentaManager / FerramentaDiscoveryService`
...
## Objetivo
Documentar e validar a camada view/client de `FerramentaMetadataDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `FerramentaMetadataDTO` no domínio Ferramenta LLM.
2. Quando a camada view/client recebe ou produz dados compatíveis com `FerramentaMetadataDTO`.
...
**When**: A camada view/client valida ou transforma `FerramentaMetadataDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `FerramentaMetadataDTO - View/Client` |
| Camada | View/Client |
