# Caso de Teste: TemplateParameterDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `TemplateParameterDTO`. Parâmetro de template usado para preenchimento dinâmico de prompts.

## Classe Testada
`com.ia.core.llm.rest.web.TemplateController`
...
## Objetivo
Documentar e validar a camada api/rest de `TemplateParameterDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateParameterDTO` no domínio Template.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `TemplateParameterDTO`.
...
**When**: A camada api/rest valida ou transforma `TemplateParameterDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TemplateParameterDTO - API/REST` |
| Camada | API/REST |
