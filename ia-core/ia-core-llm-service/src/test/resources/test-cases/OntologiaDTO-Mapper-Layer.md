# Caso de Teste: OntologiaDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `OntologiaDTO`. Representação, validação, exportação e manutenção de ontologias OWL.

## Classe Testada
`com.ia.core.llm.service.agente.mapper.OntologiaMapper`
...
## Objetivo
Documentar e validar a camada mapper de `OntologiaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `OntologiaDTO` no domínio Ontologia OWL.
2. Quando a camada mapper recebe ou produz dados compatíveis com `OntologiaDTO`.
...
**When**: A camada mapper valida ou transforma `OntologiaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OntologiaDTO - Mapper` |
| Camada | Mapper |
