# Caso de Teste: OntologiaDTO - Service

## Descrição
Caso de teste para a camada service do DTO `OntologiaDTO`. Representação, validação, exportação e manutenção de ontologias OWL.

## Classe Testada
`com.ia.core.owl.service.CoreOWLService`
...
## Objetivo
Documentar e validar a camada service de `OntologiaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `OntologiaDTO` no domínio Ontologia OWL.
2. Quando a camada service recebe ou produz dados compatíveis com `OntologiaDTO`.
...
**When**: A camada service valida ou transforma `OntologiaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OntologiaDTO - Service` |
| Camada | Service |
