# Caso de Teste: TemplateDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `TemplateDTO`. Gerenciamento de templates reutilizáveis com parâmetros e contexto.
...
## Objetivo
Documentar e validar a camada api/rest de `TemplateDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateDTO` no domínio Template.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `TemplateDTO`.
...
**When**: A camada api/rest valida ou transforma `TemplateDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TemplateDTO - API/REST` |
| Camada | API/REST |
