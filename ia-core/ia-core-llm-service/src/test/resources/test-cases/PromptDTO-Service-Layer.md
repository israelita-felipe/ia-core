# Caso de Teste: PromptDTO - Service

## Descrição
Caso de teste para a camada service do DTO `PromptDTO`. Contrato de prompt de catálogo vinculado a template e finalidade LLM.

## Classe Testada
`com.ia.core.llm.service.prompt.PromptService`
...
## Objetivo
Documentar e validar a camada service de `PromptDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `PromptDTO` no domínio Prompt.
2. Quando a camada service recebe ou produz dados compatíveis com `PromptDTO`.
...
**When**: A camada service valida ou transforma `PromptDTO`.
