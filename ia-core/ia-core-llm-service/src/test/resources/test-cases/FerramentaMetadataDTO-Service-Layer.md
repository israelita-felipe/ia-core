# Caso de Teste: FerramentaMetadataDTO - Service

## Descrição
Caso de teste para a camada service do DTO `FerramentaMetadataDTO`. Metadados resumidos de ferramentas disponíveis para seleção pelo agente/view.

## Classe Testada
`com.ia.core.llm.service.ferramenta.FerramentaDiscoveryService`
...
## Objetivo
Documentar e validar a camada service de `FerramentaMetadataDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `FerramentaMetadataDTO` no domínio Ferramenta LLM.
2. Quando a camada service recebe ou produz dados compatíveis com `FerramentaMetadataDTO`.
...
**When**: A camada service valida ou transforma `FerramentaMetadataDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `FerramentaMetadataDTO - Service` |
| Camada | Service |
