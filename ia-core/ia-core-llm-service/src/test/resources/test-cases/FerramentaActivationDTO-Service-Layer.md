# Caso de Teste: FerramentaActivationDTO - Service

## Descrição
Caso de teste para a camada service do DTO `FerramentaActivationDTO`. Ativação de ferramenta ou subferramenta para execução por agente LLM.

## Classe Testada
`com.ia.core.llm.service.ferramenta.FerramentaService`
...
## Objetivo
Documentar e validar a camada service de `FerramentaActivationDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `FerramentaActivationDTO` no domínio Ferramenta LLM.
2. Quando a camada service recebe ou produz dados compatíveis com `FerramentaActivationDTO`.
...
**When**: A camada service valida ou transforma `FerramentaActivationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `FerramentaActivationDTO - Service` |
| Camada | Service |
