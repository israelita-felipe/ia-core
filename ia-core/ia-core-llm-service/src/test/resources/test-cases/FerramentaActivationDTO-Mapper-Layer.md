# Caso de Teste: FerramentaActivationDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `FerramentaActivationDTO`. Ativação de ferramenta ou subferramenta para execução por agente LLM.

## Classe Testada
`com.ia.core.llm.service.ferramenta.FerramentaMapper`
...
## Objetivo
Documentar e validar a camada mapper de `FerramentaActivationDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `FerramentaActivationDTO` no domínio Ferramenta LLM.
2. Quando a camada mapper recebe ou produz dados compatíveis com `FerramentaActivationDTO`.
...
**When**: A camada mapper valida ou transforma `FerramentaActivationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `FerramentaActivationDTO - Mapper` |
| Camada | Mapper |
