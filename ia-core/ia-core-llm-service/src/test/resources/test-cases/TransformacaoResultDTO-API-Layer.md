# Caso de Teste: TransformacaoResultDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `TransformacaoResultDTO`. Resultado agregado de transformação de ontologia com axiomas e inconsistências.
...
## Objetivo
Documentar e validar a camada api/rest de `TransformacaoResultDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TransformacaoResultDTO` no domínio Transformação OWL.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `TransformacaoResultDTO`.
...
**When**: A camada api/rest valida ou transforma `TransformacaoResultDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TransformacaoResultDTO - API/REST` |
| Camada | API/REST |
