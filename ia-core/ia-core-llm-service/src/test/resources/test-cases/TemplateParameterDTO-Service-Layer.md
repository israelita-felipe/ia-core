# Caso de Teste: TemplateParameterDTO - Service

## Descrição
Caso de teste para a camada service do DTO `TemplateParameterDTO`. Parâmetro de template usado para preenchimento dinâmico de prompts.
...
## Objetivo
Documentar e validar a camada service de `TemplateParameterDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateParameterDTO` no domínio Template.
2. Quando a camada service recebe ou produz dados compatíveis com `TemplateParameterDTO`.
...
**When**: A camada service valida ou transforma `TemplateParameterDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TemplateParameterDTO - Service` |
| Camada | Service |
