# Caso de Teste: EstatisticasOntologiaDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `EstatisticasOntologiaDTO`. Resumo estatístico de classes, propriedades, axiomas e inconsistências de ontologia.
...
## Objetivo
Documentar e validar a camada view/client de `EstatisticasOntologiaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `EstatisticasOntologiaDTO` no domínio Ontologia OWL.
2. Quando a camada view/client recebe ou produz dados compatíveis com `EstatisticasOntologiaDTO`.
...
**When**: A camada view/client valida ou transforma `EstatisticasOntologiaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `EstatisticasOntologiaDTO - View/Client` |
| Camada | View/Client |
