# Caso de Teste: AgentSessionRequestDTO - Service Model

## Descrição
Caso de teste para a camada service model do DTO `AgentSessionRequestDTO`. Execução de mensagem do usuário em sessão orquestrada por múltiplos agentes.

## Classe Testada
`com.ia.core.llm.service.model...AgentSessionRequestDTO`
...
## Objetivo
Documentar e validar a camada service model de `AgentSessionRequestDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentSessionRequestDTO` no domínio Sessão de Agente.
2. Quando a camada service model recebe ou produz dados compatíveis com `AgentSessionRequestDTO`.
...
**When**: A camada service model valida ou transforma `AgentSessionRequestDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentSessionRequestDTO - Service Model` |
