# Caso de Teste: ChatSessionDTO - Service Model

## Descrição
Caso de teste para a camada service model do DTO `ChatSessionDTO`. Sessão de chat com agente LLM, status, usuário e histórico conversacional.

## Classe Testada
`com.ia.core.llm.service.model...ChatSessionDTO`
...
## Objetivo
Documentar e validar a camada service model de `ChatSessionDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ChatSessionDTO` no domínio Conversação Chat.
2. Quando a camada service model recebe ou produz dados compatíveis com `ChatSessionDTO`.
...
**When**: A camada service model valida ou transforma `ChatSessionDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ChatSessionDTO - Service Model` |
