# Caso de Teste: AgenteDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `AgenteDTO`. Cadastro, consulta, atualização e associação de agentes LLM com ferramentas e skills.

## Classe Testada
`com.ia.core.llm.service.agente.AgenteMapper`
...
## Objetivo
Documentar e validar a camada mapper de `AgenteDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgenteDTO` no domínio Agente LLM.
2. Quando a camada mapper recebe ou produz dados compatíveis com `AgenteDTO`.
...
**When**: A camada mapper valida ou transforma `AgenteDTO`.
