# Caso de Teste: AgentConfirmationDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `AgentConfirmationDTO`. Confirmação explícita de ação pendente em sessão multi-agente.

## Classe Testada
`Confirmação de ação de sessão`
...
## Objetivo
Documentar e validar a camada modelo/dto de `AgentConfirmationDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentConfirmationDTO` no domínio Sessão de Agente.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `AgentConfirmationDTO`.
...
**When**: A camada modelo/dto valida ou transforma `AgentConfirmationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentConfirmationDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
