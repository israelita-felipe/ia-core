# Caso de Teste: AgentSessionResponseDTO - Service

## Descrição
Caso de teste para a camada service do DTO `AgentSessionResponseDTO`. Resposta da sessão, incluindo mensagem e necessidade de confirmação do usuário.
...
## Objetivo
Documentar e validar a camada service de `AgentSessionResponseDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentSessionResponseDTO` no domínio Sessão de Agente.
2. Quando a camada service recebe ou produz dados compatíveis com `AgentSessionResponseDTO`.
...
**When**: A camada service valida ou transforma `AgentSessionResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentSessionResponseDTO - Service` |
| Camada | Service |
