# Caso de Teste: AxiomaDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `AxiomaDTO`. Representação de axioma OWL com prefixo, versão URI, expressão e consistência.
...
## Objetivo
Documentar e validar a camada view/client de `AxiomaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AxiomaDTO` no domínio Axioma OWL.
2. Quando a camada view/client recebe ou produz dados compatíveis com `AxiomaDTO`.
...
**When**: A camada view/client valida ou transforma `AxiomaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AxiomaDTO - View/Client` |
| Camada | View/Client |
