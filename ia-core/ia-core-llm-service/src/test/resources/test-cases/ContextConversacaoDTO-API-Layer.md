# Caso de Teste: ContextConversacaoDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `ContextConversacaoDTO`. Gerenciamento da ontologia de contexto da sessão conversacional.

## Classe Testada
`com.ia.core.llm.rest.web.ContextOntologyController`
...
## Objetivo
Documentar e validar a camada api/rest de `ContextConversacaoDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ContextConversacaoDTO` no domínio Contexto de Conversação.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `ContextConversacaoDTO`.
...
**When**: A camada api/rest valida ou transforma `ContextConversacaoDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ContextConversacaoDTO - API/REST` |
| Camada | API/REST |
