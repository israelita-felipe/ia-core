# Caso de Teste: TemplateParameterDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `TemplateParameterDTO`. Parâmetro de template usado para preenchimento dinâmico de prompts.

## Classe Testada
`com.ia.core.llm.service.template.TemplateParameterMapper`
...
## Objetivo
Documentar e validar a camada mapper de `TemplateParameterDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateParameterDTO` no domínio Template.
2. Quando a camada mapper recebe ou produz dados compatíveis com `TemplateParameterDTO`.
...
**When**: A camada mapper valida ou transforma `TemplateParameterDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TemplateParameterDTO - Mapper` |
| Camada | Mapper |
