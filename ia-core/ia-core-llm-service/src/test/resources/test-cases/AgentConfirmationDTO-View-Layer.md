# Caso de Teste: AgentConfirmationDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `AgentConfirmationDTO`. Confirmação explícita de ação pendente em sessão multi-agente.

## Classe Testada
`com.ia.core.llm.view.agente.session.AgentSessionManager / AgentSessionClient`

## Stack do DTO
| Camada | Componente | Status |
|--------|------------|--------|
| Model | `Confirmação de ação de sessão` | Implementado |
| Repository | `Não implementado` | Não implementado |
| Mapper | `Não implementado` | Não implementado |
| ServiceModel | [`com.ia.core.llm.service.model...AgentConfirmationDTO`](../../../../ia-core-llm-service-model/src/main/java/com/ia/core/llm/service/model/session/AgentConfirmationDTO.java) | Implementado |
...
## Objetivo
Documentar e validar a camada view/client de `AgentConfirmationDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentConfirmationDTO` no domínio Sessão de Agente.
2. Quando a camada view/client recebe ou produz dados compatíveis com `AgentConfirmationDTO`.
3. Então o comportamento deve preservar os campos `sessionId`, `confirmed`, `userMessage`.
...
**Given**: Dados válidos para `AgentConfirmationDTO` no contexto `Sessão de Agente`.
...
**When**: A camada view/client valida ou transforma `AgentConfirmationDTO`.
...
**Given**: Caso de teste documentado para `AgentConfirmationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentConfirmationDTO - View/Client` |
| Camada | View/Client |
...
- DTO: [`AgentConfirmationDTO`](../../../../ia-core-llm-service-model/src/main/java/com/ia/core/llm/service/model/session/AgentConfirmationDTO.java)
