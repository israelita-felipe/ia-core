# Caso de Teste: ChatRequestDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `ChatRequestDTO`. Solicitação de mensagem enviada ao agente LLM dentro de uma sessão.
...
## Objetivo
Documentar e validar a camada api/rest de `ChatRequestDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ChatRequestDTO` no domínio Conversação Chat.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `ChatRequestDTO`.
...
**When**: A camada api/rest valida ou transforma `ChatRequestDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ChatRequestDTO - API/REST` |
| Camada | API/REST |
