# Caso de Teste: FerramentaMetadataDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `FerramentaMetadataDTO`. Metadados resumidos de ferramentas disponíveis para seleção pelo agente/view.
...
## Objetivo
Documentar e validar a camada api/rest de `FerramentaMetadataDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `FerramentaMetadataDTO` no domínio Ferramenta LLM.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `FerramentaMetadataDTO`.
...
**When**: A camada api/rest valida ou transforma `FerramentaMetadataDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `FerramentaMetadataDTO - API/REST` |
| Camada | API/REST |
