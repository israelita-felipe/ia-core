# Caso de Teste: PromptDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `PromptDTO`. Contrato de prompt de catálogo vinculado a template e finalidade LLM.

## Classe Testada
`com.ia.core.llm.view.prompt.PromptManager / PromptClient`
...
## Objetivo
Documentar e validar a camada view/client de `PromptDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `PromptDTO` no domínio Prompt.
2. Quando a camada view/client recebe ou produz dados compatíveis com `PromptDTO`.
...
**When**: A camada view/client valida ou transforma `PromptDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PromptDTO - View/Client` |
| Camada | View/Client |
