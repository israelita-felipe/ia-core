# Caso de Teste: AxiomaDTO - Repositório

## Descrição
Caso de teste para a camada repositório do DTO `AxiomaDTO`. Representação de axioma OWL com prefixo, versão URI, expressão e consistência.

## Classe Testada
`com.ia.core.owl.service.axioma.AxiomaRepository`
...
## Objetivo
Documentar e validar a camada repositório de `AxiomaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AxiomaDTO` no domínio Axioma OWL.
2. Quando a camada repositório recebe ou produz dados compatíveis com `AxiomaDTO`.
...
**When**: A camada repositório valida ou transforma `AxiomaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AxiomaDTO - Repositório` |
| Camada | Repositório |
