# Caso de Teste: AgenteDTO - Service

## Descrição
Caso de teste para a camada service do DTO `AgenteDTO`. Cadastro, consulta, atualização e associação de agentes LLM com ferramentas e skills.

## Classe Testada
`com.ia.core.llm.service.agente.AgenteService`
...
## Objetivo
Documentar e validar a camada service de `AgenteDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgenteDTO` no domínio Agente LLM.
2. Quando a camada service recebe ou produz dados compatíveis com `AgenteDTO`.
...
**When**: A camada service valida ou transforma `AgenteDTO`.
