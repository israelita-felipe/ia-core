# Caso de Teste: TemplateDTO - Repositório

## Descrição
Caso de teste para a camada repositório do DTO `TemplateDTO`. Gerenciamento de templates reutilizáveis com parâmetros e contexto.

## Classe Testada
`com.ia.core.llm.service.template.TemplateRepository`
...
## Objetivo
Documentar e validar a camada repositório de `TemplateDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateDTO` no domínio Template.
2. Quando a camada repositório recebe ou produz dados compatíveis com `TemplateDTO`.
...
**When**: A camada repositório valida ou transforma `TemplateDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TemplateDTO - Repositório` |
| Camada | Repositório |
