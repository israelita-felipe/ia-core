# Caso de Teste: TransformacaoResultDTO - Service

## Descrição
Caso de teste para a camada service do DTO `TransformacaoResultDTO`. Resultado agregado de transformação de ontologia com axiomas e inconsistências.

## Classe Testada
`com.ia.core.owl.service.CoreOWLTransformationService`
...
## Objetivo
Documentar e validar a camada service de `TransformacaoResultDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TransformacaoResultDTO` no domínio Transformação OWL.
2. Quando a camada service recebe ou produz dados compatíveis com `TransformacaoResultDTO`.
...
**When**: A camada service valida ou transforma `TransformacaoResultDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TransformacaoResultDTO - Service` |
| Camada | Service |
