# Caso de Teste: ChatRequestDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `ChatRequestDTO`. Solicitação de mensagem enviada ao agente LLM dentro de uma sessão.
...
## Objetivo
Documentar e validar a camada mapper de `ChatRequestDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ChatRequestDTO` no domínio Conversação Chat.
2. Quando a camada mapper recebe ou produz dados compatíveis com `ChatRequestDTO`.
...
**When**: A camada mapper valida ou transforma `ChatRequestDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ChatRequestDTO - Mapper` |
| Camada | Mapper |
