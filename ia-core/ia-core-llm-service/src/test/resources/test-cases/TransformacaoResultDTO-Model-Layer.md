# Caso de Teste: TransformacaoResultDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `TransformacaoResultDTO`. Resultado agregado de transformação de ontologia com axiomas e inconsistências.

## Classe Testada
`Resultado de transformação OWL`
...
## Objetivo
Documentar e validar a camada modelo/dto de `TransformacaoResultDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TransformacaoResultDTO` no domínio Transformação OWL.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `TransformacaoResultDTO`.
...
**When**: A camada modelo/dto valida ou transforma `TransformacaoResultDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TransformacaoResultDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
