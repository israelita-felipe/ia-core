# Caso de Teste: AxiomaDTO - Service Model

## Descrição
Caso de teste para a camada service model do DTO `AxiomaDTO`. Representação de axioma OWL com prefixo, versão URI, expressão e consistência.

## Classe Testada
`com.ia.core.llm.service.model...AxiomaDTO`
...
## Objetivo
Documentar e validar a camada service model de `AxiomaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AxiomaDTO` no domínio Axioma OWL.
2. Quando a camada service model recebe ou produz dados compatíveis com `AxiomaDTO`.
...
**When**: A camada service model valida ou transforma `AxiomaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AxiomaDTO - Service Model` |
