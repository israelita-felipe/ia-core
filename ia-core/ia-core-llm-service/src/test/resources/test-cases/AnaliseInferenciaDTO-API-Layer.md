# Caso de Teste: AnaliseInferenciaDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `AnaliseInferenciaDTO`. Resultado de análise de consistência e inferências de ontologia OWL.
...
## Objetivo
Documentar e validar a camada api/rest de `AnaliseInferenciaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AnaliseInferenciaDTO` no domínio OWL Inferência.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `AnaliseInferenciaDTO`.
...
**When**: A camada api/rest valida ou transforma `AnaliseInferenciaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AnaliseInferenciaDTO - API/REST` |
| Camada | API/REST |
