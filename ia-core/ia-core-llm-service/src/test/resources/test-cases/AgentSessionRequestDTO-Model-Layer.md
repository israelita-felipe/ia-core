# Caso de Teste: AgentSessionRequestDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `AgentSessionRequestDTO`. Execução de mensagem do usuário em sessão orquestrada por múltiplos agentes.

## Classe Testada
`Requisição de sessão multi-agente`
...
## Objetivo
Documentar e validar a camada modelo/dto de `AgentSessionRequestDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentSessionRequestDTO` no domínio Sessão de Agente.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `AgentSessionRequestDTO`.
...
**When**: A camada modelo/dto valida ou transforma `AgentSessionRequestDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentSessionRequestDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
