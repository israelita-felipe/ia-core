# Caso de Teste: AgenteDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `AgenteDTO`. Cadastro, consulta, atualização e associação de agentes LLM com ferramentas e skills.

## Classe Testada
`com.ia.core.llm.view.agente.AgenteManager / AgenteClient`
...
## Objetivo
Documentar e validar a camada view/client de `AgenteDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgenteDTO` no domínio Agente LLM.
2. Quando a camada view/client recebe ou produz dados compatíveis com `AgenteDTO`.
...
**When**: A camada view/client valida ou transforma `AgenteDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgenteDTO - View/Client` |
| Camada | View/Client |
