# Caso de Teste: AgenteDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `AgenteDTO`. Cadastro, consulta, atualização e associação de agentes LLM com ferramentas e skills.

## Classe Testada
`com.ia.core.llm.rest.web.AgenteController`
...
## Objetivo
Documentar e validar a camada api/rest de `AgenteDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgenteDTO` no domínio Agente LLM.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `AgenteDTO`.
...
**When**: A camada api/rest valida ou transforma `AgenteDTO`.
