# Caso de Teste: AgentSessionRequestDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `AgentSessionRequestDTO`. Execução de mensagem do usuário em sessão orquestrada por múltiplos agentes.
...
## Objetivo
Documentar e validar a camada mapper de `AgentSessionRequestDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentSessionRequestDTO` no domínio Sessão de Agente.
2. Quando a camada mapper recebe ou produz dados compatíveis com `AgentSessionRequestDTO`.
...
**When**: A camada mapper valida ou transforma `AgentSessionRequestDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentSessionRequestDTO - Mapper` |
| Camada | Mapper |
