# Caso de Teste: ChatSessionDTO - Service

## Descrição
Caso de teste para a camada service do DTO `ChatSessionDTO`. Sessão de chat com agente LLM, status, usuário e histórico conversacional.

## Classe Testada
`com.ia.core.llm.service.chat.ChatSessionCrudService`
...
## Objetivo
Documentar e validar a camada service de `ChatSessionDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ChatSessionDTO` no domínio Conversação Chat.
2. Quando a camada service recebe ou produz dados compatíveis com `ChatSessionDTO`.
...
**When**: A camada service valida ou transforma `ChatSessionDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ChatSessionDTO - Service` |
| Camada | Service |
