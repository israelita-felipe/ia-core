# Caso de Teste: AgentSessionResponseDTO - Service Model

## Descrição
Caso de teste para a camada service model do DTO `AgentSessionResponseDTO`. Resposta da sessão, incluindo mensagem e necessidade de confirmação do usuário.

## Classe Testada
`com.ia.core.llm.service.model...AgentSessionResponseDTO`
...
## Objetivo
Documentar e validar a camada service model de `AgentSessionResponseDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentSessionResponseDTO` no domínio Sessão de Agente.
2. Quando a camada service model recebe ou produz dados compatíveis com `AgentSessionResponseDTO`.
...
**When**: A camada service model valida ou transforma `AgentSessionResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentSessionResponseDTO - Service Model` |
