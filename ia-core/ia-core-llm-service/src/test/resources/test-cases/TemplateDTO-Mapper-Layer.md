# Caso de Teste: TemplateDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `TemplateDTO`. Gerenciamento de templates reutilizáveis com parâmetros e contexto.

## Classe Testada
`com.ia.core.llm.service.template.TemplateMapper`
...
## Objetivo
Documentar e validar a camada mapper de `TemplateDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateDTO` no domínio Template.
2. Quando a camada mapper recebe ou produz dados compatíveis com `TemplateDTO`.
...
**When**: A camada mapper valida ou transforma `TemplateDTO`.
