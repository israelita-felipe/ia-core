# Caso de Teste: PromptDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `PromptDTO`. Contrato de prompt de catálogo vinculado a template e finalidade LLM.

## Classe Testada
`com.ia.core.llm.service.prompt.PromptMapper`
...
## Objetivo
Documentar e validar a camada mapper de `PromptDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `PromptDTO` no domínio Prompt.
2. Quando a camada mapper recebe ou produz dados compatíveis com `PromptDTO`.
...
**When**: A camada mapper valida ou transforma `PromptDTO`.
