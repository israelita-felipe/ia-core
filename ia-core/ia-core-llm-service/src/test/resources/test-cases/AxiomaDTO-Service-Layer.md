# Caso de Teste: AxiomaDTO - Service

## Descrição
Caso de teste para a camada service do DTO `AxiomaDTO`. Representação de axioma OWL com prefixo, versão URI, expressão e consistência.

## Classe Testada
`com.ia.core.owl.service.DefaultOwlService`
...
## Objetivo
Documentar e validar a camada service de `AxiomaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AxiomaDTO` no domínio Axioma OWL.
2. Quando a camada service recebe ou produz dados compatíveis com `AxiomaDTO`.
...
**When**: A camada service valida ou transforma `AxiomaDTO`.
