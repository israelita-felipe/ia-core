# Caso de Teste: ChatRequestDTO - Service

## Descrição
Caso de teste para a camada service do DTO `ChatRequestDTO`. Solicitação de mensagem enviada ao agente LLM dentro de uma sessão.
...
## Objetivo
Documentar e validar a camada service de `ChatRequestDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ChatRequestDTO` no domínio Conversação Chat.
2. Quando a camada service recebe ou produz dados compatíveis com `ChatRequestDTO`.
...
**When**: A camada service valida ou transforma `ChatRequestDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ChatRequestDTO - Service` |
| Camada | Service |
