# Caso de Teste: TemplateDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `TemplateDTO`. Gerenciamento de templates reutilizáveis com parâmetros e contexto.

## Classe Testada
`com.ia.core.llm.model.template.Template`
...
## Objetivo
Documentar e validar a camada modelo/dto de `TemplateDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateDTO` no domínio Template.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `TemplateDTO`.
...
**When**: A camada modelo/dto valida ou transforma `TemplateDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TemplateDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
