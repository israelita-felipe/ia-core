# Caso de Teste: ChatRequestDTO - Service Model

## Descrição
Caso de teste para a camada service model do DTO `ChatRequestDTO`. Solicitação de mensagem enviada ao agente LLM dentro de uma sessão.

## Classe Testada
`com.ia.core.llm.service.model...ChatRequestDTO`
...
## Objetivo
Documentar e validar a camada service model de `ChatRequestDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ChatRequestDTO` no domínio Conversação Chat.
2. Quando a camada service model recebe ou produz dados compatíveis com `ChatRequestDTO`.
...
**When**: A camada service model valida ou transforma `ChatRequestDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ChatRequestDTO - Service Model` |
