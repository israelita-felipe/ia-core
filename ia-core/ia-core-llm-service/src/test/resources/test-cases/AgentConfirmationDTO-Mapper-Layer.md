# Caso de Teste: AgentConfirmationDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `AgentConfirmationDTO`. Confirmação explícita de ação pendente em sessão multi-agente.
...
## Objetivo
Documentar e validar a camada mapper de `AgentConfirmationDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentConfirmationDTO` no domínio Sessão de Agente.
2. Quando a camada mapper recebe ou produz dados compatíveis com `AgentConfirmationDTO`.
...
**When**: A camada mapper valida ou transforma `AgentConfirmationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentConfirmationDTO - Mapper` |
| Camada | Mapper |
