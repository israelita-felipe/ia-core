# Caso de Teste: SkillDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `SkillDTO`. Cadastro e associação de habilidades especializadas a agentes LLM.
...
## Objetivo
Documentar e validar a camada api/rest de `SkillDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `SkillDTO` no domínio Skill LLM.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `SkillDTO`.
...
**When**: A camada api/rest valida ou transforma `SkillDTO`.
