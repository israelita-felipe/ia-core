# Caso de Teste: EstatisticasOntologiaDTO - Service

## Descrição
Caso de teste para a camada service do DTO `EstatisticasOntologiaDTO`. Resumo estatístico de classes, propriedades, axiomas e inconsistências de ontologia.
...
## Objetivo
Documentar e validar a camada service de `EstatisticasOntologiaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `EstatisticasOntologiaDTO` no domínio Ontologia OWL.
2. Quando a camada service recebe ou produz dados compatíveis com `EstatisticasOntologiaDTO`.
...
**When**: A camada service valida ou transforma `EstatisticasOntologiaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `EstatisticasOntologiaDTO - Service` |
| Camada | Service |
