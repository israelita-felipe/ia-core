# Caso de Teste: TemplateParameterDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `TemplateParameterDTO`. Parâmetro de template usado para preenchimento dinâmico de prompts.

## Classe Testada
`com.ia.core.llm.model.template.TemplateParameter`
...
## Objetivo
Documentar e validar a camada modelo/dto de `TemplateParameterDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateParameterDTO` no domínio Template.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `TemplateParameterDTO`.
...
**When**: A camada modelo/dto valida ou transforma `TemplateParameterDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TemplateParameterDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
