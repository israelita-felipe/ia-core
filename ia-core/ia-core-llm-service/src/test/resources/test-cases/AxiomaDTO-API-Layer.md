# Caso de Teste: AxiomaDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `AxiomaDTO`. Representação de axioma OWL com prefixo, versão URI, expressão e consistência.
...
## Objetivo
Documentar e validar a camada api/rest de `AxiomaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AxiomaDTO` no domínio Axioma OWL.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `AxiomaDTO`.
...
**When**: A camada api/rest valida ou transforma `AxiomaDTO`.
