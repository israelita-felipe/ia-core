# Caso de Teste: SkillDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `SkillDTO`. Cadastro e associação de habilidades especializadas a agentes LLM.

## Classe Testada
`com.ia.core.llm.model.skill.Skill`
...
## Objetivo
Documentar e validar a camada modelo/dto de `SkillDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `SkillDTO` no domínio Skill LLM.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `SkillDTO`.
...
**When**: A camada modelo/dto valida ou transforma `SkillDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SkillDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
