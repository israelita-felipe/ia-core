# Caso de Teste: AgentSessionRequestDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `AgentSessionRequestDTO`. Execução de mensagem do usuário em sessão orquestrada por múltiplos agentes.
...
## Objetivo
Documentar e validar a camada view/client de `AgentSessionRequestDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentSessionRequestDTO` no domínio Sessão de Agente.
2. Quando a camada view/client recebe ou produz dados compatíveis com `AgentSessionRequestDTO`.
...
**When**: A camada view/client valida ou transforma `AgentSessionRequestDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentSessionRequestDTO - View/Client` |
| Camada | View/Client |
