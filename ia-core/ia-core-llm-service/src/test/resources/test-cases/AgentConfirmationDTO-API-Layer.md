# Caso de Teste: AgentConfirmationDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `AgentConfirmationDTO`. Confirmação explícita de ação pendente em sessão multi-agente.
...
## Objetivo
Documentar e validar a camada api/rest de `AgentConfirmationDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentConfirmationDTO` no domínio Sessão de Agente.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `AgentConfirmationDTO`.
...
**When**: A camada api/rest valida ou transforma `AgentConfirmationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentConfirmationDTO - API/REST` |
| Camada | API/REST |
