# Caso de Teste: FerramentaActivationDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `FerramentaActivationDTO`. Ativação de ferramenta ou subferramenta para execução por agente LLM.
...
## Objetivo
Documentar e validar a camada api/rest de `FerramentaActivationDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `FerramentaActivationDTO` no domínio Ferramenta LLM.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `FerramentaActivationDTO`.
...
**When**: A camada api/rest valida ou transforma `FerramentaActivationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `FerramentaActivationDTO - API/REST` |
| Camada | API/REST |
