# Caso de Teste: FerramentaActivationDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `FerramentaActivationDTO`. Ativação de ferramenta ou subferramenta para execução por agente LLM.

## Classe Testada
`com.ia.core.llm.model.ferramenta.Ferramenta`
...
## Objetivo
Documentar e validar a camada modelo/dto de `FerramentaActivationDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `FerramentaActivationDTO` no domínio Ferramenta LLM.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `FerramentaActivationDTO`.
...
**When**: A camada modelo/dto valida ou transforma `FerramentaActivationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `FerramentaActivationDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
