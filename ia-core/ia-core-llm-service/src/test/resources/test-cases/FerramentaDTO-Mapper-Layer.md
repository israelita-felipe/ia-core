# Caso de Teste: FerramentaDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `FerramentaDTO`. Cadastro, descoberta e configuração de ferramentas reutilizáveis por agentes.
...
## Objetivo
Documentar e validar a camada mapper de `FerramentaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `FerramentaDTO` no domínio Ferramenta LLM.
2. Quando a camada mapper recebe ou produz dados compatíveis com `FerramentaDTO`.
...
**When**: A camada mapper valida ou transforma `FerramentaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `FerramentaDTO - Mapper` |
| Camada | Mapper |
