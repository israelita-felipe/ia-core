# Caso de Teste: AxiomaDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `AxiomaDTO`. Representação de axioma OWL com prefixo, versão URI, expressão e consistência.

## Classe Testada
`com.ia.core.owl.service.axioma.AxiomaMapper`
...
## Objetivo
Documentar e validar a camada mapper de `AxiomaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AxiomaDTO` no domínio Axioma OWL.
2. Quando a camada mapper recebe ou produz dados compatíveis com `AxiomaDTO`.
...
**When**: A camada mapper valida ou transforma `AxiomaDTO`.
