# Caso de Teste: SkillDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `SkillDTO`. Cadastro e associação de habilidades especializadas a agentes LLM.

## Classe Testada
`com.ia.core.llm.service.skill.SkillMapper`
...
## Objetivo
Documentar e validar a camada mapper de `SkillDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `SkillDTO` no domínio Skill LLM.
2. Quando a camada mapper recebe ou produz dados compatíveis com `SkillDTO`.
...
**When**: A camada mapper valida ou transforma `SkillDTO`.
