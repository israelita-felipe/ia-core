# Caso de Teste: AgentSessionResponseDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `AgentSessionResponseDTO`. Resposta da sessão, incluindo mensagem e necessidade de confirmação do usuário.
...
## Objetivo
Documentar e validar a camada mapper de `AgentSessionResponseDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentSessionResponseDTO` no domínio Sessão de Agente.
2. Quando a camada mapper recebe ou produz dados compatíveis com `AgentSessionResponseDTO`.
...
**When**: A camada mapper valida ou transforma `AgentSessionResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentSessionResponseDTO - Mapper` |
| Camada | Mapper |
