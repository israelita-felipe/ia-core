# Caso de Teste: PromptDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `PromptDTO`. Contrato de prompt de catálogo vinculado a template e finalidade LLM.

## Classe Testada
`com.ia.core.llm.model.prompt.Prompt`
...
## Objetivo
Documentar e validar a camada modelo/dto de `PromptDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `PromptDTO` no domínio Prompt.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `PromptDTO`.
...
**When**: A camada modelo/dto valida ou transforma `PromptDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PromptDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
