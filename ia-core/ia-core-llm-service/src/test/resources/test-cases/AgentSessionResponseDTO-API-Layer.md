# Caso de Teste: AgentSessionResponseDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `AgentSessionResponseDTO`. Resposta da sessão, incluindo mensagem e necessidade de confirmação do usuário.
...
## Objetivo
Documentar e validar a camada api/rest de `AgentSessionResponseDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentSessionResponseDTO` no domínio Sessão de Agente.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `AgentSessionResponseDTO`.
...
**When**: A camada api/rest valida ou transforma `AgentSessionResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentSessionResponseDTO - API/REST` |
| Camada | API/REST |
