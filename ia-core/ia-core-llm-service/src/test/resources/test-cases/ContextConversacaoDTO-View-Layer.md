# Caso de Teste: ContextConversacaoDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `ContextConversacaoDTO`. Gerenciamento da ontologia de contexto da sessão conversacional.

## Classe Testada
`com.ia.core.llm.view.agenteconversacional.AgenteConversacionalManager / AgenteConversacionalClient`

## Stack do DTO
| Camada | Componente | Status |
|--------|------------|--------|
| Model | `com.ia.core.llm.model.agente.ContextoConversacao` | Implementado |
| Repository | `com.ia.core.llm.service.repository.ContextoConversacaoRepository` | Implementado |
| Mapper | `com.ia.core.llm.service.agente.mapper.ContextoConversacaoMapper` | Implementado |
| ServiceModel | [`com.ia.core.llm.service.model...ContextConversacaoDTO`](../../../../ia-core-llm-service-model/src/main/java/com/ia/core/llm/service/model/agente/ContextConversacaoDTO.java) | Implementado |
| Service | `com.ia.core.llm.service.agente.ContextoConversacaoService` | Implementado |
| API/REST | `com.ia.core.llm.rest.web.ContextOntologyController` | Implementado |
| View/Client | `com.ia.core.llm.view.agenteconversacional.AgenteConversacionalManager / AgenteConversacionalClient` | Implementado |

## Objetivo
Documentar e validar a camada view/client de `ContextConversacaoDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ContextConversacaoDTO` no domínio Contexto de Conversação.
2. Quando a camada view/client recebe ou produz dados compatíveis com `ContextConversacaoDTO`.
3. Então o comportamento deve preservar os campos `sessionId`, `userId`, `dominio`, `ontologia`, `dataInicio`, `ultimaAtividade`, `totalAxiomasExtraidos`, `ontologiaConsistente`, `inconsistenciasCorrigidas`.
...
**Given**: Dados válidos para `ContextConversacaoDTO` no contexto `Contexto de Conversação`.
...
**When**: A camada view/client valida ou transforma `ContextConversacaoDTO`.
...
**Given**: Caso de teste documentado para `ContextConversacaoDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ContextConversacaoDTO - View/Client` |
| Camada | View/Client |
| Tipo de teste | Unitário/Integração por camada |
| Domínio | Contexto de Conversação |
...
- DTO: [`ContextConversacaoDTO`](../../../../ia-core-llm-service-model/src/main/java/com/ia/core/llm/service/model/agente/ContextConversacaoDTO.java)
