# Caso de Teste: AgenteDTOTest

## Descrição
Testa o DTO de transferência de dados AgenteDTO, verificando a construção correta, validações, e métodos de utilidade como clone e copy.

## Classe Testada
`com.ia.core.llm.service.model.agente.AgenteDTO`

## Fluxo do Teste
1. Dado um objeto AgenteDTO com todos os campos preenchidos
2. Quando o objeto é construído usando builder pattern
3. Então deve manter a integridade dos dados
4. E deve permitir operações de clone e copy

## Cenários
- Construção de AgenteDTO com builder pattern
- Validação de campos obrigatórios
- Teste de método clone()
- Teste de método copy()
- Verificação de constantes CAMPOS
- Teste de métodos de busca por campos
- Verificação de campos de identificação
- Verificação de campos de configuração
