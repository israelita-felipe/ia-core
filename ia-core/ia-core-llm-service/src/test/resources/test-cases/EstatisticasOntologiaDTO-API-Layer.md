# Caso de Teste: EstatisticasOntologiaDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `EstatisticasOntologiaDTO`. Resumo estatístico de classes, propriedades, axiomas e inconsistências de ontologia.
...
## Objetivo
Documentar e validar a camada api/rest de `EstatisticasOntologiaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `EstatisticasOntologiaDTO` no domínio Ontologia OWL.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `EstatisticasOntologiaDTO`.
...
**When**: A camada api/rest valida ou transforma `EstatisticasOntologiaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `EstatisticasOntologiaDTO - API/REST` |
| Camada | API/REST |
