# Caso de Teste: EstatisticasOntologiaDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `EstatisticasOntologiaDTO`. Resumo estatístico de classes, propriedades, axiomas e inconsistências de ontologia.
...
## Objetivo
Documentar e validar a camada mapper de `EstatisticasOntologiaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `EstatisticasOntologiaDTO` no domínio Ontologia OWL.
2. Quando a camada mapper recebe ou produz dados compatíveis com `EstatisticasOntologiaDTO`.
...
**When**: A camada mapper valida ou transforma `EstatisticasOntologiaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `EstatisticasOntologiaDTO - Mapper` |
| Camada | Mapper |
