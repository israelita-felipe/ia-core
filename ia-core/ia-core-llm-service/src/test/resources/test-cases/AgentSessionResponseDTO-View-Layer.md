# Caso de Teste: AgentSessionResponseDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `AgentSessionResponseDTO`. Resposta da sessão, incluindo mensagem e necessidade de confirmação do usuário.
...
## Objetivo
Documentar e validar a camada view/client de `AgentSessionResponseDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgentSessionResponseDTO` no domínio Sessão de Agente.
2. Quando a camada view/client recebe ou produz dados compatíveis com `AgentSessionResponseDTO`.
...
**When**: A camada view/client valida ou transforma `AgentSessionResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgentSessionResponseDTO - View/Client` |
| Camada | View/Client |
