# Caso de Teste: TemplateDTO - Service Model

## Descrição
Caso de teste para a camada service model do DTO `TemplateDTO`. Gerenciamento de templates reutilizáveis com parâmetros e contexto.

## Classe Testada
`com.ia.core.llm.service.model...TemplateDTO`
...
## Objetivo
Documentar e validar a camada service model de `TemplateDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateDTO` no domínio Template.
2. Quando a camada service model recebe ou produz dados compatíveis com `TemplateDTO`.
...
**When**: A camada service model valida ou transforma `TemplateDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TemplateDTO - Service Model` |
