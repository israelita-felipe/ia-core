# Caso de Teste: ChatSessionDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `ChatSessionDTO`. Sessão de chat com agente LLM, status, usuário e histórico conversacional.

## Classe Testada
`com.ia.core.llm.rest.web.ChatSessionController`
...
## Objetivo
Documentar e validar a camada api/rest de `ChatSessionDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ChatSessionDTO` no domínio Conversação Chat.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `ChatSessionDTO`.
...
**When**: A camada api/rest valida ou transforma `ChatSessionDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ChatSessionDTO - API/REST` |
| Camada | API/REST |
