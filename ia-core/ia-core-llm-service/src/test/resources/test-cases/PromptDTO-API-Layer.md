# Caso de Teste: PromptDTO - API/REST

## Descrição
Caso de teste para a camada api/rest do DTO `PromptDTO`. Contrato de prompt de catálogo vinculado a template e finalidade LLM.

## Classe Testada
`com.ia.core.llm.rest.web.PromptController`
...
## Objetivo
Documentar e validar a camada api/rest de `PromptDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `PromptDTO` no domínio Prompt.
2. Quando a camada api/rest recebe ou produz dados compatíveis com `PromptDTO`.
...
**When**: A camada api/rest valida ou transforma `PromptDTO`.
