# Caso de Teste: TemplateParameterDTO - Service Model

## Descrição
Caso de teste para a camada service model do DTO `TemplateParameterDTO`. Parâmetro de template usado para preenchimento dinâmico de prompts.

## Classe Testada
`com.ia.core.llm.service.model...TemplateParameterDTO`
...
## Objetivo
Documentar e validar a camada service model de `TemplateParameterDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `TemplateParameterDTO` no domínio Template.
2. Quando a camada service model recebe ou produz dados compatíveis com `TemplateParameterDTO`.
...
**When**: A camada service model valida ou transforma `TemplateParameterDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `TemplateParameterDTO - Service Model` |
