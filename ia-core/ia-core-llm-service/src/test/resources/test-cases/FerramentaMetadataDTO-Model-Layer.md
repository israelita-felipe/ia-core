# Caso de Teste: FerramentaMetadataDTO - Modelo/DTO

## Descrição
Caso de teste para a camada modelo/dto do DTO `FerramentaMetadataDTO`. Metadados resumidos de ferramentas disponíveis para seleção pelo agente/view.

## Classe Testada
`Metadados de descoberta de Ferramenta`
...
## Objetivo
Documentar e validar a camada modelo/dto de `FerramentaMetadataDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `FerramentaMetadataDTO` no domínio Ferramenta LLM.
2. Quando a camada modelo/dto recebe ou produz dados compatíveis com `FerramentaMetadataDTO`.
...
**When**: A camada modelo/dto valida ou transforma `FerramentaMetadataDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `FerramentaMetadataDTO - Modelo/DTO` |
| Camada | Modelo/DTO |
