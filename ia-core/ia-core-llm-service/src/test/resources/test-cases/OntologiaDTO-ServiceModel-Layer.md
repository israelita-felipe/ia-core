# Caso de Teste: OntologiaDTO - Service Model

## Descrição
Caso de teste para a camada service model do DTO `OntologiaDTO`. Representação, validação, exportação e manutenção de ontologias OWL.

## Classe Testada
`com.ia.core.llm.service.model...OntologiaDTO`
...
## Objetivo
Documentar e validar a camada service model de `OntologiaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `OntologiaDTO` no domínio Ontologia OWL.
2. Quando a camada service model recebe ou produz dados compatíveis com `OntologiaDTO`.
...
**When**: A camada service model valida ou transforma `OntologiaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OntologiaDTO - Service Model` |
