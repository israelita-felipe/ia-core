# Caso de Teste: AgenteDTO - Repositório

## Descrição
Caso de teste para a camada repositório do DTO `AgenteDTO`. Cadastro, consulta, atualização e associação de agentes LLM com ferramentas e skills.

## Classe Testada
`com.ia.core.llm.service.agente.AgenteRepository`
...
## Objetivo
Documentar e validar a camada repositório de `AgenteDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AgenteDTO` no domínio Agente LLM.
2. Quando a camada repositório recebe ou produz dados compatíveis com `AgenteDTO`.
...
**When**: A camada repositório valida ou transforma `AgenteDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AgenteDTO - Repositório` |
| Camada | Repositório |
