# Caso de Teste: ChatRequestDTO - View/Client

## Descrição
Caso de teste para a camada view/client do DTO `ChatRequestDTO`. Solicitação de mensagem enviada ao agente LLM dentro de uma sessão.
...
## Objetivo
Documentar e validar a camada view/client de `ChatRequestDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `ChatRequestDTO` no domínio Conversação Chat.
2. Quando a camada view/client recebe ou produz dados compatíveis com `ChatRequestDTO`.
...
**When**: A camada view/client valida ou transforma `ChatRequestDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ChatRequestDTO - View/Client` |
| Camada | View/Client |
