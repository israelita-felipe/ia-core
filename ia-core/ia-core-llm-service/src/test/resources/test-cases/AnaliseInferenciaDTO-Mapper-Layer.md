# Caso de Teste: AnaliseInferenciaDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `AnaliseInferenciaDTO`. Resultado de análise de consistência e inferências de ontologia OWL.
...
## Objetivo
Documentar e validar a camada mapper de `AnaliseInferenciaDTO` dentro da stack completa do `ia-core-llm-*`.

## Fluxo do Teste
1. Dado o contexto do DTO `AnaliseInferenciaDTO` no domínio OWL Inferência.
2. Quando a camada mapper recebe ou produz dados compatíveis com `AnaliseInferenciaDTO`.
...
**When**: A camada mapper valida ou transforma `AnaliseInferenciaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `AnaliseInferenciaDTO - Mapper` |
| Camada | Mapper |
