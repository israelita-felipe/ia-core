...
@Tag("unit")
@DisplayName("TemplateDTO Tests")
class TemplateDTOTest {
  private static final List<String> FIELDS = List.of("titulo", "identificador", "conteudo", "exigeContexto", "parametros");

  @Test
  @DisplayName("Should create DTO with declared fields")
  void shouldCreateDtoWithDeclaredFields() throws Exception {
    Object dto = DtoTestSupport.createDto(TemplateDTO.class, FIELDS);

    assertThat(dto).isNotNull();
    for (String field : FIELDS) {
      assertThat(DtoTestSupport.getValue(dto, field)).as(field).isEqualTo(DtoTestSupport.valueForField(TemplateDTO.class.getDeclaredField(field).getType()));
    }
  }
  ...
  void shouldExposeCloneObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCloneObject(TemplateDTO.class, dto, FIELDS);
  }
  ...
  void shouldExposeSearchRequestFactoryWhenSupported() throws Exception {
    DtoTestSupport.assertSearchRequest(TemplateDTO.class);
  }
}
