...
@Tag("unit")
@DisplayName("AxiomaDTO Tests")
class AxiomaDTOTest {
  private static final List<String> FIELDS = List.of("uri", "prefix", "uriVersion", "expressao", "consistente", "inferido", "ativo");

  @Test
  @DisplayName("Should create DTO with declared fields")
  void shouldCreateDtoWithDeclaredFields() throws Exception {
    Object dto = DtoTestSupport.createDto(AxiomaDTO.class, FIELDS);

    assertThat(dto).isNotNull();
    for (String field : FIELDS) {
      assertThat(DtoTestSupport.getValue(dto, field)).as(field).isEqualTo(DtoTestSupport.valueForField(AxiomaDTO.class.getDeclaredField(field).getType()));
    }
  }
  ...
  void shouldExposeCloneObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCloneObject(AxiomaDTO.class, dto, FIELDS);
  }
  ...
  void shouldExposeSearchRequestFactoryWhenSupported() throws Exception {
    DtoTestSupport.assertSearchRequest(AxiomaDTO.class);
  }
}
