...
@Tag("unit")
@DisplayName("TransformacaoResultDTO Tests")
class TransformacaoResultDTOTest {
  private static final List<String> FIELDS = List.of("axiomas", "inconsistencias", "sucesso", "mensagem", "analise");

  @Test
  @DisplayName("Should create DTO with declared fields")
  void shouldCreateDtoWithDeclaredFields() throws Exception {
    Object dto = DtoTestSupport.createDto(TransformacaoResultDTO.class, FIELDS);

    assertThat(dto).isNotNull();
    for (String field : FIELDS) {
      assertThat(DtoTestSupport.getValue(dto, field)).as(field).isEqualTo(DtoTestSupport.valueForField(TransformacaoResultDTO.class.getDeclaredField(field).getType()));
    }
  }
  ...
  void shouldExposeCloneObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCloneObject(TransformacaoResultDTO.class, dto, FIELDS);
  }
  ...
  void shouldExposeCopyObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCopyObject(TransformacaoResultDTO.class, dto, FIELDS);
  }

  @Test
  @DisplayName("Should expose CAMPOS constants when present")
  void shouldExposeCamposConstantsWhenPresent() throws Exception {
    DtoTestSupport.assertCampos(TransformacaoResultDTO.class, FIELDS);
  }
  ...
  void shouldExposeSearchRequestFactoryWhenSupported() throws Exception {
    DtoTestSupport.assertSearchRequest(TransformacaoResultDTO.class);
  }
}
