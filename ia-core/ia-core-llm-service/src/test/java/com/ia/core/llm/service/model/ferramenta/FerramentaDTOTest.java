...
@Tag("unit")
@DisplayName("FerramentaDTO Tests")
class FerramentaDTOTest {
  private static final List<String> FIELDS = List.of("titulo", "descricao", "tipo", "identificador", "moduloOrigem", "ativo", "descobertaAutomatica", "instrucoes", "template", "subFerramentas");

  @Test
  @DisplayName("Should create DTO with declared fields")
  void shouldCreateDtoWithDeclaredFields() throws Exception {
    Object dto = DtoTestSupport.createDto(FerramentaDTO.class, FIELDS);

    assertThat(dto).isNotNull();
    for (String field : FIELDS) {
      assertThat(DtoTestSupport.getValue(dto, field)).as(field).isEqualTo(DtoTestSupport.valueForField(FerramentaDTO.class.getDeclaredField(field).getType()));
    }
  }
  ...
  void shouldExposeCloneObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCloneObject(FerramentaDTO.class, dto, FIELDS);
  }
  ...
  void shouldExposeCopyObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCopyObject(FerramentaDTO.class, dto, FIELDS);
  }

  @Test
  @DisplayName("Should expose CAMPOS constants when present")
  void shouldExposeCamposConstantsWhenPresent() throws Exception {
    DtoTestSupport.assertCampos(FerramentaDTO.class, FIELDS);
  }
  ...
  void shouldExposeSearchRequestFactoryWhenSupported() throws Exception {
    DtoTestSupport.assertSearchRequest(FerramentaDTO.class);
  }
}
