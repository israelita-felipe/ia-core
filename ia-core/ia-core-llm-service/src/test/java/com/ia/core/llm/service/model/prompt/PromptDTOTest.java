...
@Tag("unit")
@DisplayName("PromptDTO Tests")
class PromptDTOTest {
  private static final List<String> FIELDS = List.of("finalidade", "titulo", "entrada", "template");

  @Test
  @DisplayName("Should create DTO with declared fields")
  void shouldCreateDtoWithDeclaredFields() throws Exception {
    Object dto = DtoTestSupport.createDto(PromptDTO.class, FIELDS);

    assertThat(dto).isNotNull();
    for (String field : FIELDS) {
      assertThat(DtoTestSupport.getValue(dto, field)).as(field).isEqualTo(DtoTestSupport.valueForField(PromptDTO.class.getDeclaredField(field).getType()));
    }
  }
  ...
  void shouldExposeCloneObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCloneObject(PromptDTO.class, dto, FIELDS);
  }
  ...
  void shouldExposeSearchRequestFactoryWhenSupported() throws Exception {
    DtoTestSupport.assertSearchRequest(PromptDTO.class);
  }
}
