...
@Tag("unit")
@DisplayName("AgentConfirmationDTO Tests")
class AgentConfirmationDTOTest {
  private static final List<String> FIELDS = List.of("sessionId", "confirmed", "userMessage");

  @Test
  @DisplayName("Should create DTO with declared fields")
  void shouldCreateDtoWithDeclaredFields() throws Exception {
    Object dto = DtoTestSupport.createDto(AgentConfirmationDTO.class, FIELDS);

    assertThat(dto).isNotNull();
    for (String field : FIELDS) {
      assertThat(DtoTestSupport.getValue(dto, field)).as(field).isEqualTo(DtoTestSupport.valueForField(AgentConfirmationDTO.class.getDeclaredField(field).getType()));
    }
  }
  ...
  void shouldExposeCloneObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCloneObject(AgentConfirmationDTO.class, dto, FIELDS);
  }
  ...
  void shouldExposeCopyObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCopyObject(AgentConfirmationDTO.class, dto, FIELDS);
  }

  @Test
  @DisplayName("Should expose CAMPOS constants when present")
  void shouldExposeCamposConstantsWhenPresent() throws Exception {
    DtoTestSupport.assertCampos(AgentConfirmationDTO.class, FIELDS);
  }
  ...
  void shouldExposeSearchRequestFactoryWhenSupported() throws Exception {
    DtoTestSupport.assertSearchRequest(AgentConfirmationDTO.class);
  }
}
