...
@Tag("unit")
@DisplayName("ChatSessionDTO Tests")
class ChatSessionDTOTest {
  private static final List<String> FIELDS = List.of("sessionId", "titulo", "dataInicio", "dataFim", "status", "agente", "usuarioId");

  @Test
  @DisplayName("Should create DTO with declared fields")
  void shouldCreateDtoWithDeclaredFields() throws Exception {
    Object dto = DtoTestSupport.createDto(ChatSessionDTO.class, FIELDS);

    assertThat(dto).isNotNull();
    for (String field : FIELDS) {
      assertThat(DtoTestSupport.getValue(dto, field)).as(field).isEqualTo(DtoTestSupport.valueForField(ChatSessionDTO.class.getDeclaredField(field).getType()));
    }
  }
  ...
  void shouldExposeCloneObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCloneObject(ChatSessionDTO.class, dto, FIELDS);
  }
  ...
  void shouldExposeCopyObjectContractWhenSupported() throws Exception {
    ...
    DtoTestSupport.assertCopyObject(ChatSessionDTO.class, dto, FIELDS);
  }

  @Test
  @DisplayName("Should expose CAMPOS constants when present")
  void shouldExposeCamposConstantsWhenPresent() throws Exception {
    DtoTestSupport.assertCampos(ChatSessionDTO.class, FIELDS);
  }
  ...
  void shouldExposeSearchRequestFactoryWhenSupported() throws Exception {
    DtoTestSupport.assertSearchRequest(ChatSessionDTO.class);
  }
}
