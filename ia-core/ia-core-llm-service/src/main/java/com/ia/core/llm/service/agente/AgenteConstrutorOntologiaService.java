...
public class AgenteConstrutorOntologiaService extends AgenteConversacionalOntologiaService {
  ...
  public ResultadoConstrucaoOntologiaDTO iniciarConstrucao(RequisicaoConstrucaoOntologiaDTO requisicao) {
    ...
    return ResultadoConstrucaoOntologiaDTO.builder()
        .jobId(jobId)
        .status("QUEUED")
        .build();
  }
  ...
  public ResultadoConstrucaoOntologiaDTO obterProgresso(@ToolParam(description = "ID do job de construção de ontologia") String jobId) {
    ...
    return ResultadoConstrucaoOntologiaDTO.builder()
        ...
        .statistics(EstatisticasOntologiaDTO.builder()
            .classeCount(job.getClassCount())
            .axiomCount(job.getAxiomCount())
            .objectPropertyCount(job.getPropertyCount())
  ...
  public ResultadoConstrucaoOntologiaDTO obterResultado(@ToolParam(description = "ID do job de construção de ontologia") String jobId) {
    JobConstrucao job = jobsAtivos.get(jobId);
    if (job == null) {
      return ResultadoConstrucaoOntologiaDTO.builder()
          .jobId(jobId)
  ...
  public ResultadoConstrucaoOntologiaDTO cancelarJob(@ToolParam(description = "ID do job de construção de ontologia") String jobId) {
    JobConstrucao job = jobsAtivos.get(jobId);
    if (job == null) {
      return ResultadoConstrucaoOntologiaDTO.builder()
          .jobId(jobId)
  ...
  private void executarJob(String sessionId,JobConstrucao job) {
    ...
    try {
      ...
      log.info("Job concluído com sucesso: jobId={}, axiomas={}", job.getJobId(), axiomasValidados.size());
  ...
  private List<String> extrairElementos(String sessionId,RequisicaoConstrucaoOntologiaDTO requisicao) {
    log.debug("Iniciando extração de elementos do corpus: tamanho={} caracteres", requisicao.getCorpus().length());
  ...
  private List<AxiomaDTO> validarERefinar(String sessionId, JobConstrucao job, List<AxiomaDTO> axiomas) {
    ...
    for (AxiomaDTO axioma : axiomas) {
      ResultadoValidacaoDTO resultado = getOwlService().validarAxioma(axioma);
  ...
  private static class JobConstrucao {
    ...
    private RequisicaoConstrucaoOntologiaDTO requisicao;
    ...
    public RequisicaoConstrucaoOntologiaDTO getRequisicao() { return requisicao; }
