# Caso de Teste: QuartzJobDTO - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobDTO`.
...
## Objetivo
Documentar e validar a camada service de `QuartzJobDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `QuartzJobDTO`.
...
**When**: A camada service valida ou transforma `QuartzJobDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobDTO - Service` |
| Camada | Service |
