# Caso de Teste: QuartzJobDTO - Mapper

## Descrição
Caso de teste para a camada mapper da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `QuartzJobDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `QuartzJobDTO`.
...
**When**: A camada mapper valida ou transforma `QuartzJobDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobDTO - Mapper` |
| Camada | Mapper |
