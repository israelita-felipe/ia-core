# Caso de Teste: PeriodicidadeDTO - View

## Descrição
Caso de teste para a camada view da classe `com.ia.core.quartz.service.model.periodicidade.dto.PeriodicidadeDTO`.
...
## Objetivo
Documentar e validar a camada view de `PeriodicidadeDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `PeriodicidadeDTO`.
...
**When**: A camada view valida ou transforma `PeriodicidadeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PeriodicidadeDTO - View` |
| Camada | View |
