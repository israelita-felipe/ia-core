# Caso de Teste: SchedulerConfigDTO - View

## Descrição
Caso de teste para a camada view da classe `com.ia.core.quartz.service.model.scheduler.dto.SchedulerConfigDTO`.
...
## Objetivo
Documentar e validar a camada view de `SchedulerConfigDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Agendamento` no domínio Quartz.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `SchedulerConfigDTO`.
...
**When**: A camada view valida ou transforma `SchedulerConfigDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SchedulerConfigDTO - View` |
| Camada | View |
