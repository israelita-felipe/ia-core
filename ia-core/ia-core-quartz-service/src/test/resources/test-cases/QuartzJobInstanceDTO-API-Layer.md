# Caso de Teste: QuartzJobInstanceDTO - API

## Descrição
Caso de teste para a camada api da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobInstanceDTO`.
...
## Objetivo
Documentar e validar a camada api de `QuartzJobInstanceDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `QuartzJobInstanceDTO`.
...
**When**: A camada api valida ou transforma `QuartzJobInstanceDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobInstanceDTO - API` |
| Camada | API |
