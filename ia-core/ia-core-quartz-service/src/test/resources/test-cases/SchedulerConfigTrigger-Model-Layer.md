# Caso de Teste: SchedulerConfigTrigger - Model

## Descrição
Caso de teste para a camada model da classe `com.ia.core.quartz.model.scheduler.SchedulerConfigTrigger`.
...
## Objetivo
Documentar e validar a camada model de `SchedulerConfigTrigger` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Agendamento` no domínio Quartz.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `SchedulerConfigTrigger`.
...
**When**: A camada model valida ou transforma `SchedulerConfigTrigger`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SchedulerConfigTrigger - Model` |
| Camada | Model |
