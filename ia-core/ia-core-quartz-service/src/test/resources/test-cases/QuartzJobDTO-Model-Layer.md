# Caso de Teste: QuartzJobDTO - Model

## Descrição
Caso de teste para a camada model da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobDTO`.
...
## Objetivo
Documentar e validar a camada model de `QuartzJobDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `QuartzJobDTO`.
...
**When**: A camada model valida ou transforma `QuartzJobDTO`.
