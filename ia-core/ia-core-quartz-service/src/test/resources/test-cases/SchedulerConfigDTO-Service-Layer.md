# Caso de Teste: SchedulerConfigDTO - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.scheduler.dto.SchedulerConfigDTO`.
...
## Objetivo
Documentar e validar a camada service de `SchedulerConfigDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Agendamento` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `SchedulerConfigDTO`.
...
**When**: A camada service valida ou transforma `SchedulerConfigDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SchedulerConfigDTO - Service` |
| Camada | Service |
