...
## Objetivo
Documentar e validar a camada model de `QuartzJobSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `QuartzJobSearchRequest`.
