# Caso de Teste: SchedulerConfigTriggerDTO - Model

## Descrição
Caso de teste para a camada model da classe `com.ia.core.quartz.service.model.scheduler.dto.triggers.SchedulerConfigTriggerDTO`.
...
## Objetivo
Documentar e validar a camada model de `SchedulerConfigTriggerDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Agendamento` no domínio Quartz.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `SchedulerConfigTriggerDTO`.
...
**When**: A camada model valida ou transforma `SchedulerConfigTriggerDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SchedulerConfigTriggerDTO - Model` |
| Camada | Model |
