# Caso de Teste: SchedulerConfigTriggerDTO - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.scheduler.dto.triggers.SchedulerConfigTriggerDTO`.
...
## Objetivo
Documentar e validar a camada service de `SchedulerConfigTriggerDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Agendamento` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `SchedulerConfigTriggerDTO`.
...
**When**: A camada service valida ou transforma `SchedulerConfigTriggerDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SchedulerConfigTriggerDTO - Service` |
| Camada | Service |
