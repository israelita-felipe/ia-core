# Caso de Teste: QuartzJobTriggerSearchRequest - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobTriggerSearchRequest`.
...
## Objetivo
Documentar e validar a camada service de `QuartzJobTriggerSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `QuartzJobTriggerSearchRequest`.
...
**When**: A camada service valida ou transforma `QuartzJobTriggerSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobTriggerSearchRequest - Service` |
| Camada | Service |
