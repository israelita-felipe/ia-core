# Caso de Teste: QuartzJobTriggerDTO - API

## Descrição
Caso de teste para a camada api da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobTriggerDTO`.
...
## Objetivo
Documentar e validar a camada api de `QuartzJobTriggerDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `QuartzJobTriggerDTO`.
...
**When**: A camada api valida ou transforma `QuartzJobTriggerDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobTriggerDTO - API` |
| Camada | API |
