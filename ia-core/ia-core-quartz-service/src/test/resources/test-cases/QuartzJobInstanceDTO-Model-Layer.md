# Caso de Teste: QuartzJobInstanceDTO - Model

## Descrição
Caso de teste para a camada model da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobInstanceDTO`.
...
## Objetivo
Documentar e validar a camada model de `QuartzJobInstanceDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `QuartzJobInstanceDTO`.
...
**When**: A camada model valida ou transforma `QuartzJobInstanceDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobInstanceDTO - Model` |
| Camada | Model |
