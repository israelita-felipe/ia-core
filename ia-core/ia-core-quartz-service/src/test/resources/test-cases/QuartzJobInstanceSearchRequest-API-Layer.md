# Caso de Teste: QuartzJobInstanceSearchRequest - API

## Descrição
Caso de teste para a camada api da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobInstanceSearchRequest`.
...
## Objetivo
Documentar e validar a camada api de `QuartzJobInstanceSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `QuartzJobInstanceSearchRequest`.
...
**When**: A camada api valida ou transforma `QuartzJobInstanceSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobInstanceSearchRequest - API` |
| Camada | API |
