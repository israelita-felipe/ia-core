# Caso de Teste: RecorrenciaDTO - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.recorrencia.dto.RecorrenciaDTO`.
...
## Objetivo
Documentar e validar a camada service de `RecorrenciaDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `RecorrenciaDTO`.
...
**When**: A camada service valida ou transforma `RecorrenciaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RecorrenciaDTO - Service` |
| Camada | Service |
