# Caso de Teste: QuartzJobInstanceSearchRequest - Mapper

## Descrição
Caso de teste para a camada mapper da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobInstanceSearchRequest`.
...
## Objetivo
Documentar e validar a camada mapper de `QuartzJobInstanceSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `QuartzJobInstanceSearchRequest`.
...
**When**: A camada mapper valida ou transforma `QuartzJobInstanceSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobInstanceSearchRequest - Mapper` |
| Camada | Mapper |
