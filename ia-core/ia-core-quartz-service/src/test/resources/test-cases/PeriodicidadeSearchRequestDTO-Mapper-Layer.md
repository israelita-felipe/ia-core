# Caso de Teste: PeriodicidadeSearchRequest - Mapper

## Descrição
Caso de teste para a camada mapper da classe `com.ia.core.quartz.service.model.periodicidade.dto.PeriodicidadeSearchRequest`.
...
## Objetivo
Documentar e validar a camada mapper de `PeriodicidadeSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `PeriodicidadeSearchRequest`.
...
**When**: A camada mapper valida ou transforma `PeriodicidadeSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PeriodicidadeSearchRequest - Mapper` |
| Camada | Mapper |
