# Caso de Teste: IntervaloTemporalDTO - Mapper

## Descrição
Caso de teste para a camada mapper da classe `com.ia.core.quartz.service.model.periodicidade.dto.IntervaloTemporalDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `IntervaloTemporalDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `IntervaloTemporalDTO`.
...
**When**: A camada mapper valida ou transforma `IntervaloTemporalDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `IntervaloTemporalDTO - Mapper` |
| Camada | Mapper |
