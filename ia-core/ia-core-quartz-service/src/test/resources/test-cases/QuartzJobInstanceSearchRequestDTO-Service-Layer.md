# Caso de Teste: QuartzJobInstanceSearchRequest - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobInstanceSearchRequest`.
...
## Objetivo
Documentar e validar a camada service de `QuartzJobInstanceSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `QuartzJobInstanceSearchRequest`.
...
**When**: A camada service valida ou transforma `QuartzJobInstanceSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobInstanceSearchRequest - Service` |
| Camada | Service |
