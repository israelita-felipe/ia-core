# Caso de Teste: IntervaloTemporalDTO - API

## Descrição
Caso de teste para a camada api da classe `com.ia.core.quartz.service.model.periodicidade.dto.IntervaloTemporalDTO`.
...
## Objetivo
Documentar e validar a camada api de `IntervaloTemporalDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `IntervaloTemporalDTO`.
...
**When**: A camada api valida ou transforma `IntervaloTemporalDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `IntervaloTemporalDTO - API` |
| Camada | API |
