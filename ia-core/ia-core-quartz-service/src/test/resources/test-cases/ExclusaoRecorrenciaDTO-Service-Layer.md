# Caso de Teste: ExclusaoRecorrenciaDTO - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.recorrencia.dto.ExclusaoRecorrenciaDTO`.
...
## Objetivo
Documentar e validar a camada service de `ExclusaoRecorrenciaDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `ExclusaoRecorrenciaDTO`.
...
**When**: A camada service valida ou transforma `ExclusaoRecorrenciaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ExclusaoRecorrenciaDTO - Service` |
| Camada | Service |
