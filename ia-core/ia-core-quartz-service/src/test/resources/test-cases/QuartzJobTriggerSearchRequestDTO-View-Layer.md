# Caso de Teste: QuartzJobTriggerSearchRequest - View

## Descrição
Caso de teste para a camada view da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobTriggerSearchRequest`.
...
## Objetivo
Documentar e validar a camada view de `QuartzJobTriggerSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `QuartzJobTriggerSearchRequest`.
...
**When**: A camada view valida ou transforma `QuartzJobTriggerSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobTriggerSearchRequest - View` |
| Camada | View |
