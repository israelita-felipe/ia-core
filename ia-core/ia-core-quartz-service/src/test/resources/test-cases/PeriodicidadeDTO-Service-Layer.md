# Caso de Teste: PeriodicidadeDTO - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.periodicidade.dto.PeriodicidadeDTO`.
...
## Objetivo
Documentar e validar a camada service de `PeriodicidadeDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `PeriodicidadeDTO`.
...
**When**: A camada service valida ou transforma `PeriodicidadeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PeriodicidadeDTO - Service` |
| Camada | Service |
