# Caso de Teste: ExclusaoRecorrenciaDTO - API

## Descrição
Caso de teste para a camada api da classe `com.ia.core.quartz.service.model.recorrencia.dto.ExclusaoRecorrenciaDTO`.
...
## Objetivo
Documentar e validar a camada api de `ExclusaoRecorrenciaDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `ExclusaoRecorrenciaDTO`.
...
**When**: A camada api valida ou transforma `ExclusaoRecorrenciaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ExclusaoRecorrenciaDTO - API` |
| Camada | API |
