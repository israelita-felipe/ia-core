# Caso de Teste: QuartzJobTriggerDTO - Mapper

## Descrição
Caso de teste para a camada mapper da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobTriggerDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `QuartzJobTriggerDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `QuartzJobTriggerDTO`.
...
**When**: A camada mapper valida ou transforma `QuartzJobTriggerDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobTriggerDTO - Mapper` |
| Camada | Mapper |
