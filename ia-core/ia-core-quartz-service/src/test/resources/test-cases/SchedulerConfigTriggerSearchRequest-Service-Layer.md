# Caso de Teste: SchedulerConfigTriggerSearchRequest - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.scheduler.dto.triggers.SchedulerConfigTriggerSearchRequest`.
...
| Service | Serviço específico quando aplicável | Implementado |
...
## Objetivo
Documentar e validar a camada service de `SchedulerConfigTriggerSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Agendamento` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `SchedulerConfigTriggerSearchRequest`.
...
**When**: A camada service processa o componente.
...
**When**: A camada service valida ou transforma `SchedulerConfigTriggerSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SchedulerConfigTriggerSearchRequest - Service` |
| Camada | Service |
