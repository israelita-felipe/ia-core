# Caso de Teste: QuartzJobTriggerSearchRequest - Mapper

## Descrição
Caso de teste para a camada mapper da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobTriggerSearchRequest`.
...
## Objetivo
Documentar e validar a camada mapper de `QuartzJobTriggerSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `QuartzJobTriggerSearchRequest`.
...
**When**: A camada mapper valida ou transforma `QuartzJobTriggerSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobTriggerSearchRequest - Mapper` |
| Camada | Mapper |
