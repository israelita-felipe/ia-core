# Caso de Teste: ExclusaoRecorrenciaDTO - View

## Descrição
Caso de teste para a camada view da classe `com.ia.core.quartz.service.model.recorrencia.dto.ExclusaoRecorrenciaDTO`.
...
## Objetivo
Documentar e validar a camada view de `ExclusaoRecorrenciaDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `ExclusaoRecorrenciaDTO`.
...
**When**: A camada view valida ou transforma `ExclusaoRecorrenciaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ExclusaoRecorrenciaDTO - View` |
| Camada | View |
