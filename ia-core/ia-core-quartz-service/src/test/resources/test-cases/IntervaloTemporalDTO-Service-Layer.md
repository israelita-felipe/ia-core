# Caso de Teste: IntervaloTemporalDTO - Service

## Descrição
Caso de teste para a camada service da classe `com.ia.core.quartz.service.model.periodicidade.dto.IntervaloTemporalDTO`.
...
## Objetivo
Documentar e validar a camada service de `IntervaloTemporalDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `IntervaloTemporalDTO`.
...
**When**: A camada service valida ou transforma `IntervaloTemporalDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `IntervaloTemporalDTO - Service` |
| Camada | Service |
