# Caso de Teste: MonthConverter - Model

## Descrição
Caso de teste para a camada model da classe `com.ia.core.quartz.model.periodicidade.converter.MonthConverter`.
...
|--------|------------|--------|
| Model | [com.ia.core.quartz.model.periodicidade.converter.MonthConverter](ia-core-quartz-model/src/main/java/com/ia/core/quartz/model/periodicidade/converter/MonthConverter.java) | Implementado |
...
| ServiceModel | [com.ia.core.quartz.model.periodicidade.converter.MonthConverter](ia-core-quartz-model/src/main/java/com/ia/core/quartz/model/periodicidade/converter/MonthConverter.java) | Não implementado |
...
## Objetivo
Documentar e validar a camada model de `MonthConverter` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `MonthConverter`.
...
- **Cenário 1: converter mês nulo para coluna do banco**
  - **Given**: uma instância de `MonthConverter`.
  - **When**: `convertToDatabaseColumn(null)` é executado.
  - **Then**: o resultado deve ser `null`.

- **Cenário 2: converter todos os meses válidos para coluna do banco**
  - **Given**: todos os valores de `java.time.Month`.
  - **When**: cada mês é convertido com `convertToDatabaseColumn`.
  - **Then**: o valor retornado deve ser igual a `Month#getValue`, preservando a faixa de 1 a 12.
...
- **Cenário 4: converter inteiros válidos para atributo da entidade**
  - **Given**: os valores de 1 a 12.
  - **When**: cada valor é convertido com `convertToEntityAttribute`.
  - **Then**: o `Month` retornado deve corresponder ao mês equivalente.

- **Cenário 5: rejeitar inteiro fora da faixa mensal**
...
**Given**: Dados válidos para `MonthConverter` no contexto `Periodicidade/Recorrência`.
...
**When**: A camada model valida ou transforma `MonthConverter`.
...
**Given**: Caso de teste documentado para `MonthConverter`.
...
| Campo | Valor |
|-------|-------|
| Componente | `MonthConverter - Model` |
| Camada | Model |
...
- Classe: [`MonthConverter`](ia-core-quartz-model/src/main/java/com/ia/core/quartz/model/periodicidade/converter/MonthConverter.java)
- Teste: [`MonthConverterTest`](ia-core-quartz-test/src/test/java/com/ia/core/quartz/test/MonthConverterTest.java)
