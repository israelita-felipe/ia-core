# Caso de Teste: QuartzJobInstanceSearchRequest - Model

## Descrição
Caso de teste para a camada model da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobInstanceSearchRequest`.
...
## Objetivo
Documentar e validar a camada model de `QuartzJobInstanceSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `QuartzJobInstanceSearchRequest`.
...
**When**: A camada model valida ou transforma `QuartzJobInstanceSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobInstanceSearchRequest - Model` |
| Camada | Model |
