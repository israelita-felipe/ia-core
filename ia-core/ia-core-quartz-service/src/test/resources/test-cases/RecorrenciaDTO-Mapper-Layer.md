# Caso de Teste: RecorrenciaDTO - Mapper

## Descrição
Caso de teste para a camada mapper da classe `com.ia.core.quartz.service.model.recorrencia.dto.RecorrenciaDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `RecorrenciaDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `RecorrenciaDTO`.
...
**When**: A camada mapper valida ou transforma `RecorrenciaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RecorrenciaDTO - Mapper` |
| Camada | Mapper |
