# Caso de Teste: QuartzJobInstanceDTO - View

## Descrição
Caso de teste para a camada view da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobInstanceDTO`.
...
## Objetivo
Documentar e validar a camada view de `QuartzJobInstanceDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `QuartzJobInstanceDTO`.
...
**When**: A camada view valida ou transforma `QuartzJobInstanceDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobInstanceDTO - View` |
| Camada | View |
