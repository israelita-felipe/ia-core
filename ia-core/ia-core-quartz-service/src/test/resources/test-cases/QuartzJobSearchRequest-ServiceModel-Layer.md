# Caso de Teste: QuartzJobSearchRequest - ServiceModel

## Descrição
Caso de teste para a camada servicemodel da classe `com.ia.core.quartz.service.model.job.dto.QuartzJobSearchRequest`.
...
| ServiceModel | [com.ia.core.quartz.service.model.job.dto.QuartzJobSearchRequest](ia-core-quartz-service-model/src/main/java/com/ia/core/quartz/service/model/job/dto/QuartzJobSearchRequest.java) | Implementado |
...
## Objetivo
Documentar e validar a camada servicemodel de `QuartzJobSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Jobs` no domínio Quartz.
2. Quando a camada servicemodel recebe, produz ou transforma dados compatíveis com `QuartzJobSearchRequest`.
...
**Given**: Dados válidos para `QuartzJobSearchRequest` no contexto `Jobs`.
...
**When**: A camada servicemodel valida ou transforma `QuartzJobSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `QuartzJobSearchRequest - ServiceModel` |
| Camada | ServiceModel |
