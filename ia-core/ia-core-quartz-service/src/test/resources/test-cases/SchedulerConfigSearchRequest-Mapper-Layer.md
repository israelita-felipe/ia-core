# Caso de Teste: SchedulerConfigSearchRequest - Mapper

## Descrição
Caso de teste para a camada mapper da classe `com.ia.core.quartz.service.model.scheduler.dto.SchedulerConfigSearchRequest`.
...
## Objetivo
Documentar e validar a camada mapper de `SchedulerConfigSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Agendamento` no domínio Quartz.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `SchedulerConfigSearchRequest`.
...
**When**: A camada mapper valida ou transforma `SchedulerConfigSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SchedulerConfigSearchRequest - Mapper` |
| Camada | Mapper |
