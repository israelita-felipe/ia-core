# Caso de Teste: ExclusaoRecorrenciaDTO - Model

## Descrição
Caso de teste para a camada model da classe `com.ia.core.quartz.service.model.recorrencia.dto.ExclusaoRecorrenciaDTO`.
...
## Objetivo
Documentar e validar a camada model de `ExclusaoRecorrenciaDTO` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Periodicidade/Recorrência` no domínio Quartz.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `ExclusaoRecorrenciaDTO`.
...
**When**: A camada model valida ou transforma `ExclusaoRecorrenciaDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `ExclusaoRecorrenciaDTO - Model` |
| Camada | Model |
