# Caso de Teste: SchedulerConfigTriggerSearchRequest - Mapper

## Descrição
Caso de teste para a camada mapper da classe `com.ia.core.quartz.service.model.scheduler.dto.triggers.SchedulerConfigTriggerSearchRequest`.
...
## Objetivo
Documentar e validar a camada mapper de `SchedulerConfigTriggerSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Agendamento` no domínio Quartz.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `SchedulerConfigTriggerSearchRequest`.
...
**When**: A camada mapper processa o componente.
...
**When**: A camada mapper valida ou transforma `SchedulerConfigTriggerSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SchedulerConfigTriggerSearchRequest - Mapper` |
| Camada | Mapper |
