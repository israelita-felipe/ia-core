# Caso de Teste: SchedulerConfigTriggerSearchRequest - API

## Descrição
Caso de teste para a camada api da classe `com.ia.core.quartz.service.model.scheduler.dto.triggers.SchedulerConfigTriggerSearchRequest`.
...
## Objetivo
Documentar e validar a camada api de `SchedulerConfigTriggerSearchRequest` dentro da stack de agendamento e recorrência do `ia-core-quartz-*`.

## Fluxo do Teste
1. Dado o contexto de agendamento `Agendamento` no domínio Quartz.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `SchedulerConfigTriggerSearchRequest`.
...
**When**: A camada api processa o componente.
...
**When**: A camada api valida ou transforma `SchedulerConfigTriggerSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `SchedulerConfigTriggerSearchRequest - API` |
| Camada | API |
