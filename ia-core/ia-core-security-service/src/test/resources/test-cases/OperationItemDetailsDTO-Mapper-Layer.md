# Caso de Teste: OperationItemDetails - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `OperationItemDetails`.
...
## Objetivo
Documentar e validar a camada mapper de `OperationItemDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `OperationItemDetails`.
...
**When**: A camada mapper valida ou transforma `OperationItemDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OperationItemDetails - Mapper` |
| Camada | Mapper |
