# Caso de Teste: LogOperationDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `LogOperationDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `LogOperationDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `LogOperationDTO`.
...
**When**: A camada mapper valida ou transforma `LogOperationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDTO - Mapper` |
| Camada | Mapper |
