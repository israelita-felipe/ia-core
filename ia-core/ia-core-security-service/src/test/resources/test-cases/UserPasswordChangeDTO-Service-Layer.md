# Caso de Teste: UserPasswordChangeDTO - Service

## Descrição
Caso de teste para a camada service do DTO `UserPasswordChangeDTO`.
...
## Objetivo
Documentar e validar a camada service de `UserPasswordChangeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `UserPasswordChangeDTO`.
...
**When**: A camada service valida ou transforma `UserPasswordChangeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPasswordChangeDTO - Service` |
| Camada | Service |
