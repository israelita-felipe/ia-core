# Caso de Teste: UserPasswordChangeDTO - API

## Descrição
Caso de teste para a camada api do DTO `UserPasswordChangeDTO`.
...
## Objetivo
Documentar e validar a camada api de `UserPasswordChangeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `UserPasswordChangeDTO`.
...
**When**: A camada api valida ou transforma `UserPasswordChangeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPasswordChangeDTO - API` |
| Camada | API |
