# Caso de Teste: LogOperationDTO - API

## Descrição
Caso de teste para a camada api do DTO `LogOperationDTO`.
...
## Objetivo
Documentar e validar a camada api de `LogOperationDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `LogOperationDTO`.
...
**When**: A camada api valida ou transforma `LogOperationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDTO - API` |
| Camada | API |
