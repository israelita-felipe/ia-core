# Caso de Teste: LogOperationSearchRequest - View

## Descrição
Caso de teste para a camada view do DTO `LogOperationSearchRequest`.
...
## Objetivo
Documentar e validar a camada view de `LogOperationSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `LogOperationSearchRequest`.
...
**When**: A camada view valida ou transforma `LogOperationSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationSearchRequest - View` |
| Camada | View |
