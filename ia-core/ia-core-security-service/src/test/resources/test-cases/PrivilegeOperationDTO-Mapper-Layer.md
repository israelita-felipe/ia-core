# Caso de Teste: PrivilegeOperationDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `PrivilegeOperationDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `PrivilegeOperationDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `PrivilegeOperationDTO`.
...
**When**: A camada mapper valida ou transforma `PrivilegeOperationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeOperationDTO - Mapper` |
| Camada | Mapper |
