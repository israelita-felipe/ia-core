# Caso de Teste: UserRoleDTO - API

## Descrição
Caso de teste para a camada api do DTO `UserRoleDTO`.
...
## Objetivo
Documentar e validar a camada api de `UserRoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `UserRoleDTO`.
...
**When**: A camada api valida ou transforma `UserRoleDTO`.
