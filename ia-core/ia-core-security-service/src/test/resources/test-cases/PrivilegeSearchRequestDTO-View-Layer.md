# Caso de Teste: PrivilegeSearchRequest - View

## Descrição
Caso de teste para a camada view do DTO `PrivilegeSearchRequest`.
...
## Objetivo
Documentar e validar a camada view de `PrivilegeSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `PrivilegeSearchRequest`.
...
**When**: A camada view valida ou transforma `PrivilegeSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeSearchRequest - View` |
| Camada | View |
