# Caso de Teste: LogOperationDetails - API

## Descrição
Caso de teste para a camada api do DTO `LogOperationDetails`.
...
## Objetivo
Documentar e validar a camada api de `LogOperationDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `LogOperationDetails`.
...
**When**: A camada api valida ou transforma `LogOperationDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDetails - API` |
| Camada | API |
