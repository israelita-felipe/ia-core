# Caso de Teste: RolePrivilegeDTO - API

## Descrição
Caso de teste para a camada api do DTO `RolePrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada api de `RolePrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `RolePrivilegeDTO`.
...
**When**: A camada api valida ou transforma `RolePrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RolePrivilegeDTO - API` |
| Camada | API |
