# Caso de Teste: UserPrivilegeDTO - Service

## Descrição
Caso de teste para a camada service do DTO `UserPrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada service de `UserPrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `UserPrivilegeDTO`.
...
**When**: A camada service valida ou transforma `UserPrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPrivilegeDTO - Service` |
| Camada | Service |
