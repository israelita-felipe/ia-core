# Caso de Teste: RoleDTO - API

## Descrição
Caso de teste para a camada api do DTO `RoleDTO`.
...
## Objetivo
Documentar e validar a camada api de `RoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `RoleDTO`.
...
**When**: A camada api valida ou transforma `RoleDTO`.
