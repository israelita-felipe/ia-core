# Caso de Teste: UserPasswordChangeDTO - Model

## Descrição
Caso de teste para a camada model do DTO `UserPasswordChangeDTO`.
...
## Objetivo
Documentar e validar a camada model de `UserPasswordChangeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `UserPasswordChangeDTO`.
...
**When**: A camada model valida ou transforma `UserPasswordChangeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPasswordChangeDTO - Model` |
| Camada | Model |
