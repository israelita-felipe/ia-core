# Caso de Teste: PrivilegeOperationContextDTO - Service

## Descrição
Caso de teste para a camada service do DTO `PrivilegeOperationContextDTO`.
...
## Objetivo
Documentar e validar a camada service de `PrivilegeOperationContextDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `PrivilegeOperationContextDTO`.
...
**When**: A camada service valida ou transforma `PrivilegeOperationContextDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeOperationContextDTO - Service` |
| Camada | Service |
