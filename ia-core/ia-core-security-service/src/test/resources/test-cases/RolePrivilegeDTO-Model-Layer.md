# Caso de Teste: RolePrivilegeDTO - Model

## Descrição
Caso de teste para a camada model do DTO `RolePrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada model de `RolePrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `RolePrivilegeDTO`.
...
**When**: A camada model valida ou transforma `RolePrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RolePrivilegeDTO - Model` |
| Camada | Model |
