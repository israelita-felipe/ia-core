# Caso de Teste: PrivilegeDTO - Service

## Descrição
Caso de teste para a camada service do DTO `PrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada service de `PrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `PrivilegeDTO`.
...
**When**: A camada service valida ou transforma `PrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeDTO - Service` |
| Camada | Service |
