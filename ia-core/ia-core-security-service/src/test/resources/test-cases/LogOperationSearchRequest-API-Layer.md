# Caso de Teste: LogOperationSearchRequest - API

## Descrição
Caso de teste para a camada api do DTO `LogOperationSearchRequest`.
...
## Objetivo
Documentar e validar a camada api de `LogOperationSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `LogOperationSearchRequest`.
...
**When**: A camada api valida ou transforma `LogOperationSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationSearchRequest - API` |
| Camada | API |
