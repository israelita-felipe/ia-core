# Caso de Teste: JwtAuthenticationResponseDTO - ServiceModel

## Descrição
Caso de teste para a camada servicemodel do DTO `JwtAuthenticationResponseDTO`.
...
|--------|------------|--------|
| Model | [com.ia.core.security.service.model.authentication.JwtAuthenticationResponseDTO](../../../../ia-core-security-service-model/src/main/java/com/ia/core/security/service/model/authentication/JwtAuthenticationResponseDTO.java) | Implementado |
| Repository | UserRepository quando aplicável | Implementado |
| Mapper | LoginTranslator | Implementado |
| ServiceModel | [com.ia.core.security.service.model.authentication.JwtAuthenticationResponseDTO](../../../../ia-core-security-service-model/src/main/java/com/ia/core/security/service/model/authentication/JwtAuthenticationResponseDTO.java) | Implementado |
| Service | CoreJwtAuthenticationService / JwtAuthenticationService | Implementado |
...
## Objetivo
Documentar e validar a camada servicemodel de `JwtAuthenticationResponseDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Autenticação e JWT` no domínio `Manter Security`.
2. Quando a camada servicemodel recebe, produz ou transforma dados compatíveis com `JwtAuthenticationResponseDTO`.
3. Então o comportamento deve preservar os campos `token, refreshToken`.
...
**Given**: Dados válidos para `JwtAuthenticationResponseDTO` no contexto `Autenticação e JWT`.
...
**When**: A camada servicemodel valida ou transforma `JwtAuthenticationResponseDTO`.
...
**Given**: Caso de teste documentado para `JwtAuthenticationResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `JwtAuthenticationResponseDTO - ServiceModel` |
| Camada | ServiceModel |
| Tipo de teste | Unitário/Integração por camada |
| Domínio | Autenticação e JWT |
...
- Classe: [`JwtAuthenticationResponseDTO`](../../../../ia-core-security-service-model/src/main/java/com/ia/core/security/service/model/authentication/JwtAuthenticationResponseDTO.java)
