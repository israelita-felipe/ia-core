# Caso de Teste: PrivilegeOperationDTO - Service

## Descrição
Caso de teste para a camada service do DTO `PrivilegeOperationDTO`.
...
## Objetivo
Documentar e validar a camada service de `PrivilegeOperationDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `PrivilegeOperationDTO`.
...
**When**: A camada service valida ou transforma `PrivilegeOperationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeOperationDTO - Service` |
| Camada | Service |
