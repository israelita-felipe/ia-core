# Caso de Teste: PrivilegeDTO - View

## Descrição
Caso de teste para a camada view do DTO `PrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada view de `PrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `PrivilegeDTO`.
...
**When**: A camada view valida ou transforma `PrivilegeDTO`.
