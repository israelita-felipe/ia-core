# Caso de Teste: LogOperationDTO - View

## Descrição
Caso de teste para a camada view do DTO `LogOperationDTO`.
...
## Objetivo
Documentar e validar a camada view de `LogOperationDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `LogOperationDTO`.
...
**When**: A camada view valida ou transforma `LogOperationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDTO - View` |
| Camada | View |
