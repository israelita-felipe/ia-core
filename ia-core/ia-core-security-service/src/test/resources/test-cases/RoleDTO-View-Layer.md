# Caso de Teste: RoleDTO - View

## Descrição
Caso de teste para a camada view do DTO `RoleDTO`.
...
## Objetivo
Documentar e validar a camada view de `RoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `RoleDTO`.
...
**When**: A camada view valida ou transforma `RoleDTO`.
