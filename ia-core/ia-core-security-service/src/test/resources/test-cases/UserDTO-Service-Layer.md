# Caso de Teste: UserDTO - Service

## Descrição
Caso de teste para a camada service do DTO `UserDTO`.
...
## Objetivo
Documentar e validar a camada service de `UserDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `UserDTO`.
...
**When**: A camada service valida ou transforma `UserDTO`.
