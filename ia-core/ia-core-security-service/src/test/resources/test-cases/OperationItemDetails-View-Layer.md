# Caso de Teste: OperationItemDetails - View

## Descrição
Caso de teste para a camada view do DTO `OperationItemDetails`.
...
## Objetivo
Documentar e validar a camada view de `OperationItemDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `OperationItemDetails`.
...
**When**: A camada view valida ou transforma `OperationItemDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OperationItemDetails - View` |
| Camada | View |
