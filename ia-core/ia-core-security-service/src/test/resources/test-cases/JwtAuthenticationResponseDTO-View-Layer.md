# Caso de Teste: JwtAuthenticationResponseDTO - View

## Descrição
Caso de teste para a camada view do DTO `JwtAuthenticationResponseDTO`.
...
## Objetivo
Documentar e validar a camada view de `JwtAuthenticationResponseDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Autenticação e JWT` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `JwtAuthenticationResponseDTO`.
...
**When**: A camada view valida ou transforma `JwtAuthenticationResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `JwtAuthenticationResponseDTO - View` |
| Camada | View |
