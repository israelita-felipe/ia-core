# Caso de Teste: RoleSearchRequest - Model

## Descrição
Caso de teste para a camada model do DTO `RoleSearchRequest`.
...
## Objetivo
Documentar e validar a camada model de `RoleSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `RoleSearchRequest`.
...
**When**: A camada model valida ou transforma `RoleSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RoleSearchRequest - Model` |
| Camada | Model |
