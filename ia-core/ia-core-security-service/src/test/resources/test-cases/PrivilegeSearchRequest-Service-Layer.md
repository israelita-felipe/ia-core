# Caso de Teste: PrivilegeSearchRequest - Service

## Descrição
Caso de teste para a camada service do DTO `PrivilegeSearchRequest`.
...
## Objetivo
Documentar e validar a camada service de `PrivilegeSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `PrivilegeSearchRequest`.
...
**When**: A camada service valida ou transforma `PrivilegeSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeSearchRequest - Service` |
| Camada | Service |
