# Caso de Teste: JwtAuthenticationResponseDTO - API

## Descrição
Caso de teste para a camada api do DTO `JwtAuthenticationResponseDTO`.
...
## Objetivo
Documentar e validar a camada api de `JwtAuthenticationResponseDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Autenticação e JWT` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `JwtAuthenticationResponseDTO`.
...
**When**: A camada api valida ou transforma `JwtAuthenticationResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `JwtAuthenticationResponseDTO - API` |
| Camada | API |
