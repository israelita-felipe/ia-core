...
## Objetivo
Documentar e validar a camada repository de `LogOperationDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada repository recebe, produz ou transforma dados compatíveis com `LogOperationDTO`.
