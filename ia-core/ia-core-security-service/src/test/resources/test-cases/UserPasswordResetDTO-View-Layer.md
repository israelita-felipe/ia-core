# Caso de Teste: UserPasswordResetDTO - View

## Descrição
Caso de teste para a camada view do DTO `UserPasswordResetDTO`.
...
## Objetivo
Documentar e validar a camada view de `UserPasswordResetDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `UserPasswordResetDTO`.
...
**When**: A camada view valida ou transforma `UserPasswordResetDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPasswordResetDTO - View` |
| Camada | View |
