# Caso de Teste: UserPrivilegeDTO - Model

## Descrição
Caso de teste para a camada model do DTO `UserPrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada model de `UserPrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `UserPrivilegeDTO`.
...
**When**: A camada model valida ou transforma `UserPrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPrivilegeDTO - Model` |
| Camada | Model |
