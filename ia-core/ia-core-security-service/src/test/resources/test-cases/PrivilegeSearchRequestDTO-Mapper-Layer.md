# Caso de Teste: PrivilegeSearchRequest - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `PrivilegeSearchRequest`.
...
## Objetivo
Documentar e validar a camada mapper de `PrivilegeSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `PrivilegeSearchRequest`.
...
**When**: A camada mapper valida ou transforma `PrivilegeSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeSearchRequest - Mapper` |
| Camada | Mapper |
