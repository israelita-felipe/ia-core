# Caso de Teste: PrivilegeOperationContextDTO - API

## Descrição
Caso de teste para a camada api do DTO `PrivilegeOperationContextDTO`.
...
## Objetivo
Documentar e validar a camada api de `PrivilegeOperationContextDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `PrivilegeOperationContextDTO`.
...
**When**: A camada api valida ou transforma `PrivilegeOperationContextDTO`.
...
**When**: A camada api executa o fluxo.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeOperationContextDTO - API` |
| Camada | API |
