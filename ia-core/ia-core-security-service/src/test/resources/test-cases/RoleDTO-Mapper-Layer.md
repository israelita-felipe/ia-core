# Caso de Teste: RoleDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `RoleDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `RoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `RoleDTO`.
...
**When**: A camada mapper valida ou transforma `RoleDTO`.
