# Caso de Teste: PrivilegeDTO - API

## Descrição
Caso de teste para a camada api do DTO `PrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada api de `PrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `PrivilegeDTO`.
...
**When**: A camada api valida ou transforma `PrivilegeDTO`.
