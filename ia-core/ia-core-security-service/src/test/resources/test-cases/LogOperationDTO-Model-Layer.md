# Caso de Teste: LogOperationDTO - Model

## Descrição
Caso de teste para a camada model do DTO `LogOperationDTO`.
...
## Objetivo
Documentar e validar a camada model de `LogOperationDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `LogOperationDTO`.
...
**When**: A camada model valida ou transforma `LogOperationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDTO - Model` |
| Camada | Model |
