# Caso de Teste: RolePrivilegeDTO - View

## Descrição
Caso de teste para a camada view do DTO `RolePrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada view de `RolePrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `RolePrivilegeDTO`.
...
**When**: A camada view valida ou transforma `RolePrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RolePrivilegeDTO - View` |
| Camada | View |
