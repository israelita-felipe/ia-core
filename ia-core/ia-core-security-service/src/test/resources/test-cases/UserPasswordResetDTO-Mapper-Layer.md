# Caso de Teste: UserPasswordResetDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `UserPasswordResetDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `UserPasswordResetDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `UserPasswordResetDTO`.
...
**When**: A camada mapper valida ou transforma `UserPasswordResetDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPasswordResetDTO - Mapper` |
| Camada | Mapper |
