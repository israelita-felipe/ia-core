# Caso de Teste: JwtAuthenticationResponseDTO - Service

## Descrição
Caso de teste para a camada service do DTO `JwtAuthenticationResponseDTO`.
...
## Objetivo
Documentar e validar a camada service de `JwtAuthenticationResponseDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Autenticação e JWT` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `JwtAuthenticationResponseDTO`.
...
**When**: A camada service valida ou transforma `JwtAuthenticationResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `JwtAuthenticationResponseDTO - Service` |
| Camada | Service |
