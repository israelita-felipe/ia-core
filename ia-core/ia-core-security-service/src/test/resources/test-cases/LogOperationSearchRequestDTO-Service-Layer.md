# Caso de Teste: LogOperationSearchRequest - Service

## Descrição
Caso de teste para a camada service do DTO `LogOperationSearchRequest`.
...
## Objetivo
Documentar e validar a camada service de `LogOperationSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `LogOperationSearchRequest`.
...
**When**: A camada service valida ou transforma `LogOperationSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationSearchRequest - Service` |
| Camada | Service |
