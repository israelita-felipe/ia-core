# Caso de Teste: RolePrivilegeDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `RolePrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `RolePrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `RolePrivilegeDTO`.
...
**When**: A camada mapper valida ou transforma `RolePrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RolePrivilegeDTO - Mapper` |
| Camada | Mapper |
