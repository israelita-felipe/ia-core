# Caso de Teste: OperationItemDetails - Service

## Descrição
Caso de teste para a camada service do DTO `OperationItemDetails`.
...
## Objetivo
Documentar e validar a camada service de `OperationItemDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `OperationItemDetails`.
...
**When**: A camada service valida ou transforma `OperationItemDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OperationItemDetails - Service` |
| Camada | Service |
