# Caso de Teste: UserDTO - Model

## Descrição
Caso de teste para a camada model do DTO `UserDTO`.
...
## Objetivo
Documentar e validar a camada model de `UserDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `UserDTO`.
...
**When**: A camada model valida ou transforma `UserDTO`.
