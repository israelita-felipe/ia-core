# Caso de Teste: UserPasswordChangeDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `UserPasswordChangeDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `UserPasswordChangeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `UserPasswordChangeDTO`.
...
**When**: A camada mapper valida ou transforma `UserPasswordChangeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPasswordChangeDTO - Mapper` |
| Camada | Mapper |
