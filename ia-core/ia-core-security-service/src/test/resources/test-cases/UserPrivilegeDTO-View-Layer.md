# Caso de Teste: UserPrivilegeDTO - View

## Descrição
Caso de teste para a camada view do DTO `UserPrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada view de `UserPrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `UserPrivilegeDTO`.
...
**When**: A camada view valida ou transforma `UserPrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPrivilegeDTO - View` |
| Camada | View |
