# Caso de Teste: UserRoleDTO - View

## Descrição
Caso de teste para a camada view do DTO `UserRoleDTO`.
...
## Objetivo
Documentar e validar a camada view de `UserRoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `UserRoleDTO`.
...
**When**: A camada view valida ou transforma `UserRoleDTO`.
