# Caso de Teste: LogOperationDetails - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `LogOperationDetails`.
...
## Objetivo
Documentar e validar a camada mapper de `LogOperationDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `LogOperationDetails`.
...
**When**: A camada mapper valida ou transforma `LogOperationDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDetails - Mapper` |
| Camada | Mapper |
