# Caso de Teste: PrivilegeOperationContextDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `PrivilegeOperationContextDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `PrivilegeOperationContextDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `PrivilegeOperationContextDTO`.
...
**When**: A camada mapper valida ou transforma `PrivilegeOperationContextDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeOperationContextDTO - Mapper` |
| Camada | Mapper |
