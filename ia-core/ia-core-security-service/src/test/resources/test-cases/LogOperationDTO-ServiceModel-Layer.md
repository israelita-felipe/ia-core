# Caso de Teste: LogOperationDTO - ServiceModel

## Descrição
Caso de teste para a camada servicemodel do DTO `LogOperationDTO`.
...
## Objetivo
Documentar e validar a camada servicemodel de `LogOperationDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada servicemodel recebe, produz ou transforma dados compatíveis com `LogOperationDTO`.
...
**When**: A camada servicemodel valida ou transforma `LogOperationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDTO - ServiceModel` |
| Camada | ServiceModel |
