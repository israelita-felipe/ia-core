# Caso de Teste: UserSearchRequest - Service

## Descrição
Caso de teste para a camada service do DTO `UserSearchRequest`.
...
## Objetivo
Documentar e validar a camada service de `UserSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `UserSearchRequest`.
...
**When**: A camada service valida ou transforma `UserSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserSearchRequest - Service` |
| Camada | Service |
