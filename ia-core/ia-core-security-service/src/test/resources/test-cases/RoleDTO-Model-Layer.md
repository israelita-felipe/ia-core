# Caso de Teste: RoleDTO - Model

## Descrição
Caso de teste para a camada model do DTO `RoleDTO`.
...
## Objetivo
Documentar e validar a camada model de `RoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `RoleDTO`.
...
**When**: A camada model valida ou transforma `RoleDTO`.
