# Caso de Teste: UserSearchRequest - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `UserSearchRequest`.
...
## Objetivo
Documentar e validar a camada mapper de `UserSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `UserSearchRequest`.
...
**When**: A camada mapper valida ou transforma `UserSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserSearchRequest - Mapper` |
| Camada | Mapper |
