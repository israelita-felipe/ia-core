# Caso de Teste: UserPrivilegeDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `UserPrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `UserPrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `UserPrivilegeDTO`.
...
**When**: A camada mapper valida ou transforma `UserPrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPrivilegeDTO - Mapper` |
| Camada | Mapper |
