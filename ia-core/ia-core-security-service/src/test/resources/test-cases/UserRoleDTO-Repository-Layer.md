# Caso de Teste: UserRoleDTO - Repository

## Descrição
Caso de teste para a camada repository do DTO `UserRoleDTO`.
...
## Objetivo
Documentar e validar a camada repository de `UserRoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada repository recebe, produz ou transforma dados compatíveis com `UserRoleDTO`.
...
**When**: A camada repository valida ou transforma `UserRoleDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserRoleDTO - Repository` |
| Camada | Repository |
