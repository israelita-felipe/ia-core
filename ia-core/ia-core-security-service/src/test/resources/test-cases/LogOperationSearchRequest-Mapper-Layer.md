# Caso de Teste: LogOperationSearchRequest - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `LogOperationSearchRequest`.
...
## Objetivo
Documentar e validar a camada mapper de `LogOperationSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `LogOperationSearchRequest`.
...
**When**: A camada mapper valida ou transforma `LogOperationSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationSearchRequest - Mapper` |
| Camada | Mapper |
