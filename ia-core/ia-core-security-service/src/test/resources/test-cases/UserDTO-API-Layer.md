# Caso de Teste: UserDTO - API

## Descrição
Caso de teste para a camada api do DTO `UserDTO`.
...
## Objetivo
Documentar e validar a camada api de `UserDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `UserDTO`.
...
**When**: A camada api valida ou transforma `UserDTO`.
