# Caso de Teste: RoleSearchRequest - View

## Descrição
Caso de teste para a camada view do DTO `RoleSearchRequest`.
...
## Objetivo
Documentar e validar a camada view de `RoleSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `RoleSearchRequest`.
...
**When**: A camada view valida ou transforma `RoleSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RoleSearchRequest - View` |
| Camada | View |
