# Caso de Teste: UserPasswordResetDTO - API

## Descrição
Caso de teste para a camada api do DTO `UserPasswordResetDTO`.
...
## Objetivo
Documentar e validar a camada api de `UserPasswordResetDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `UserPasswordResetDTO`.
...
**When**: A camada api valida ou transforma `UserPasswordResetDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPasswordResetDTO - API` |
| Camada | API |
