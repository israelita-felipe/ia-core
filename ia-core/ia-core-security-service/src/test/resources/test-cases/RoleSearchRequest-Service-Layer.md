# Caso de Teste: RoleSearchRequest - Service

## Descrição
Caso de teste para a camada service do DTO `RoleSearchRequest`.
...
## Objetivo
Documentar e validar a camada service de `RoleSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `RoleSearchRequest`.
...
**When**: A camada service valida ou transforma `RoleSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RoleSearchRequest - Service` |
| Camada | Service |
