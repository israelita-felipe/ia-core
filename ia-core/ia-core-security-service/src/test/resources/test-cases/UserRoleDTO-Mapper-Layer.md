# Caso de Teste: UserRoleDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `UserRoleDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `UserRoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `UserRoleDTO`.
...
**When**: A camada mapper valida ou transforma `UserRoleDTO`.
