# Caso de Teste: PrivilegeOperationDTO - API

## Descrição
Caso de teste para a camada api do DTO `PrivilegeOperationDTO`.
...
## Objetivo
Documentar e validar a camada api de `PrivilegeOperationDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `PrivilegeOperationDTO`.
...
**When**: A camada api valida ou transforma `PrivilegeOperationDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeOperationDTO - API` |
| Camada | API |
