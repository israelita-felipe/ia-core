# Caso de Teste: UserSearchRequest - API

## Descrição
Caso de teste para a camada api do DTO `UserSearchRequest`.
...
## Objetivo
Documentar e validar a camada api de `UserSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `UserSearchRequest`.
...
**When**: A camada api valida ou transforma `UserSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserSearchRequest - API` |
| Camada | API |
