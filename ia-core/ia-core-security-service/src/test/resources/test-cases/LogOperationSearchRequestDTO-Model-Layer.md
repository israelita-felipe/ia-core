# Caso de Teste: LogOperationSearchRequest - Model

## Descrição
Caso de teste para a camada model do DTO `LogOperationSearchRequest`.
...
## Objetivo
Documentar e validar a camada model de `LogOperationSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `LogOperationSearchRequest`.
...
**When**: A camada model valida ou transforma `LogOperationSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationSearchRequest - Model` |
| Camada | Model |
