# Caso de Teste: RoleSearchRequest - API

## Descrição
Caso de teste para a camada api do DTO `RoleSearchRequest`.
...
## Objetivo
Documentar e validar a camada api de `RoleSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `RoleSearchRequest`.
...
**When**: A camada api valida ou transforma `RoleSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RoleSearchRequest - API` |
| Camada | API |
