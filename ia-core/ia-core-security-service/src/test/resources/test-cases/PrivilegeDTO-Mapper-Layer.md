# Caso de Teste: PrivilegeDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `PrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `PrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `PrivilegeDTO`.
...
**When**: A camada mapper valida ou transforma `PrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeDTO - Mapper` |
| Camada | Mapper |
