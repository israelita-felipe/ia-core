# Caso de Teste: PrivilegeSearchRequest - API

## Descrição
Caso de teste para a camada api do DTO `PrivilegeSearchRequest`.
...
## Objetivo
Documentar e validar a camada api de `PrivilegeSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `PrivilegeSearchRequest`.
...
**When**: A camada api valida ou transforma `PrivilegeSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeSearchRequest - API` |
| Camada | API |
