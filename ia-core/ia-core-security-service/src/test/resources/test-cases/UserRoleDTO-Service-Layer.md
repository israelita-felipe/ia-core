# Caso de Teste: UserRoleDTO - Service

## Descrição
Caso de teste para a camada service do DTO `UserRoleDTO`.
...
## Objetivo
Documentar e validar a camada service de `UserRoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `UserRoleDTO`.
...
**When**: A camada service valida ou transforma `UserRoleDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserRoleDTO - Service` |
| Camada | Service |
