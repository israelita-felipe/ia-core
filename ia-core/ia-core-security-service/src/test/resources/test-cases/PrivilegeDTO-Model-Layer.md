# Caso de Teste: PrivilegeDTO - Model

## Descrição
Caso de teste para a camada model do DTO `PrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada model de `PrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `PrivilegeDTO`.
...
**When**: A camada model valida ou transforma `PrivilegeDTO`.
