# Caso de Teste: OperationItemDetails - API

## Descrição
Caso de teste para a camada api do DTO `OperationItemDetails`.
...
## Objetivo
Documentar e validar a camada api de `OperationItemDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada api recebe, produz ou transforma dados compatíveis com `OperationItemDetails`.
...
**When**: A camada api valida ou transforma `OperationItemDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OperationItemDetails - API` |
| Camada | API |
