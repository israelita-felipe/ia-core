# Caso de Teste: UserPasswordResetDTO - Model

## Descrição
Caso de teste para a camada model do DTO `UserPasswordResetDTO`.
...
## Objetivo
Documentar e validar a camada model de `UserPasswordResetDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `UserPasswordResetDTO`.
...
**When**: A camada model valida ou transforma `UserPasswordResetDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserPasswordResetDTO - Model` |
| Camada | Model |
