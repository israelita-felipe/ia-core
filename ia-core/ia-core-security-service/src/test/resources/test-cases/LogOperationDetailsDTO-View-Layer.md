# Caso de Teste: LogOperationDetails - View

## Descrição
Caso de teste para a camada view do DTO `LogOperationDetails`.
...
## Objetivo
Documentar e validar a camada view de `LogOperationDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada view recebe, produz ou transforma dados compatíveis com `LogOperationDetails`.
...
**When**: A camada view valida ou transforma `LogOperationDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDetails - View` |
| Camada | View |
