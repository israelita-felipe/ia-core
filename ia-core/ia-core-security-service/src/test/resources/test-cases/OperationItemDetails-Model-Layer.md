# Caso de Teste: OperationItemDetails - Model

## Descrição
Caso de teste para a camada model do DTO `OperationItemDetails`.
...
## Objetivo
Documentar e validar a camada model de `OperationItemDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `OperationItemDetails`.
...
**When**: A camada model valida ou transforma `OperationItemDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `OperationItemDetails - Model` |
| Camada | Model |
