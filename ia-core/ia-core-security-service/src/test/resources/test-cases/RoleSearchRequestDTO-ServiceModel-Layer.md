# Caso de Teste: RoleSearchRequest - ServiceModel

## Descrição
Caso de teste para a camada servicemodel do DTO `RoleSearchRequest`.
...
## Objetivo
Documentar e validar a camada servicemodel de `RoleSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada servicemodel recebe, produz ou transforma dados compatíveis com `RoleSearchRequest`.
...
**When**: A camada servicemodel valida ou transforma `RoleSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RoleSearchRequest - ServiceModel` |
| Camada | ServiceModel |
