# Caso de Teste: RoleDTO - Service

## Descrição
Caso de teste para a camada service do DTO `RoleDTO`.
...
## Objetivo
Documentar e validar a camada service de `RoleDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `RoleDTO`.
...
**When**: A camada service valida ou transforma `RoleDTO`.
