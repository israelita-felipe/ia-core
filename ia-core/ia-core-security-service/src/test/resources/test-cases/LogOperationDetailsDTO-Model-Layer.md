# Caso de Teste: LogOperationDetails - Model

## Descrição
Caso de teste para a camada model do DTO `LogOperationDetails`.
...
## Objetivo
Documentar e validar a camada model de `LogOperationDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `LogOperationDetails`.
...
**When**: A camada model valida ou transforma `LogOperationDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDetails - Model` |
| Camada | Model |
