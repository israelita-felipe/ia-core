# Caso de Teste: LogOperationDetails - Service

## Descrição
Caso de teste para a camada service do DTO `LogOperationDetails`.
...
## Objetivo
Documentar e validar a camada service de `LogOperationDetails` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Auditoria de Operação` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `LogOperationDetails`.
...
**When**: A camada service valida ou transforma `LogOperationDetails`.
...
| Campo | Valor |
|-------|-------|
| Componente | `LogOperationDetails - Service` |
| Camada | Service |
