# Caso de Teste: JwtAuthenticationResponseDTO - Mapper

## Descrição
Caso de teste para a camada mapper do DTO `JwtAuthenticationResponseDTO`.
...
## Objetivo
Documentar e validar a camada mapper de `JwtAuthenticationResponseDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Autenticação e JWT` no domínio `Manter Security`.
2. Quando a camada mapper recebe, produz ou transforma dados compatíveis com `JwtAuthenticationResponseDTO`.
...
**When**: A camada mapper valida ou transforma `JwtAuthenticationResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `JwtAuthenticationResponseDTO - Mapper` |
| Camada | Mapper |
