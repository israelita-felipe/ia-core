# Caso de Teste: JwtAuthenticationResponseDTO - Model

## Descrição
Caso de teste para a camada model do DTO `JwtAuthenticationResponseDTO`.
...
## Objetivo
Documentar e validar a camada model de `JwtAuthenticationResponseDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Autenticação e JWT` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `JwtAuthenticationResponseDTO`.
...
**When**: A camada model valida ou transforma `JwtAuthenticationResponseDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `JwtAuthenticationResponseDTO - Model` |
| Camada | Model |
