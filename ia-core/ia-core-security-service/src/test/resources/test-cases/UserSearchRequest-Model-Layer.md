# Caso de Teste: UserSearchRequest - Model

## Descrição
Caso de teste para a camada model do DTO `UserSearchRequest`.
...
## Objetivo
Documentar e validar a camada model de `UserSearchRequest` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Usuário` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `UserSearchRequest`.
...
**When**: A camada model valida ou transforma `UserSearchRequest`.
...
| Campo | Valor |
|-------|-------|
| Componente | `UserSearchRequest - Model` |
| Camada | Model |
