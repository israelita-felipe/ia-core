# Caso de Teste: RolePrivilegeDTO - Service

## Descrição
Caso de teste para a camada service do DTO `RolePrivilegeDTO`.
...
## Objetivo
Documentar e validar a camada service de `RolePrivilegeDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Papel` no domínio `Manter Security`.
2. Quando a camada service recebe, produz ou transforma dados compatíveis com `RolePrivilegeDTO`.
...
**When**: A camada service valida ou transforma `RolePrivilegeDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `RolePrivilegeDTO - Service` |
| Camada | Service |
