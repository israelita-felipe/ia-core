# Caso de Teste: PrivilegeOperationContextDTO - Model

## Descrição
Caso de teste para a camada model do DTO `PrivilegeOperationContextDTO`.
...
## Objetivo
Documentar e validar a camada model de `PrivilegeOperationContextDTO` dentro da stack de segurança do `ia-core-security-*`.

## Fluxo do Teste
1. Dado o contexto de segurança `Privilégio` no domínio `Manter Security`.
2. Quando a camada model recebe, produz ou transforma dados compatíveis com `PrivilegeOperationContextDTO`.
...
**When**: A camada model processa o DTO.
...
**When**: A camada model valida ou transforma `PrivilegeOperationContextDTO`.
...
| Campo | Valor |
|-------|-------|
| Componente | `PrivilegeOperationContextDTO - Model` |
| Camada | Model |
