...
public abstract class DefaultSecuredBaseService<T extends BaseEntity, D extends DTO<?>>
  ...
  public void afterSave(D original, D saved,
    ...
    if (saved != null && config.getEventPublisher() != null) {
      registerTransactionSynchronization(() -> publishEvent(saved, operationType),
                                         operationType.name(),
  ...
  public void afterDelete(Long id, D dto)
    ...
    if (dto != null && config.getEventPublisher() != null) {
      registerTransactionSynchronization(() -> publishEvent(dto, CrudOperationType.DELETED),
                                         "DELETE",
  ...
  /**
   * Registra sincronização de transação para publicação de eventos.
   *
   * @param eventPublisher ação de publicação do evento
   * @param operationType nome do tipo de operação
   * @param entityName nome da entidade
   * @param id identificador da entidade (opcional)
   */
  private void registerTransactionSynchronization(Runnable eventPublisher,
  ...
  public Set<Functionality> registryFunctionalities(FunctionalityManager functionalityManager) {
    registryFunctionalitiesAudit();

    return java.util.stream.Stream.<java.util.function.Supplier<Set<Functionality>>>of(
        () -> CountSecuredBaseService.super.registryFunctionalities(functionalityManager),
        () -> DeleteSecuredBaseService.super.registryFunctionalities(functionalityManager),
        () -> FindSecuredBaseService.super.registryFunctionalities(functionalityManager),
        () -> ListSecuredBaseService.super.registryFunctionalities(functionalityManager),
        () -> SaveSecuredBaseService.super.registryFunctionalities(functionalityManager)
    ).flatMap(f->f.get().stream()).collect(java.util.stream.Collectors.toSet());
  }

  /**
   * Registra auditoria para o registro de funcionalidades.
   */
  private void registryFunctionalitiesAudit() {
