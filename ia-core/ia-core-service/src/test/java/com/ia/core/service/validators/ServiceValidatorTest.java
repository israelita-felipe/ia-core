...
class ServiceValidatorTest {
    ...
    class TestesRegistry {
        ...
        void deveRegistrarHasValidation() {
            ...
            // Quando
            ServiceValidatorRegistry result = validator.registry(hasValidation);
        ...
        void deveRetornarAcaoDeRemocao() {
            // Dado
            HasValidation<TestDTO> hasValidation = mock(HasValidation.class);
            ServiceValidatorRegistry registry = validator.registry(hasValidation);

            // Quando
            registry.remove();

            // Então
            assertThat(validator.getHasValidation()).doesNotContain(hasValidation);
        }
    }

    // Implementação de teste
    static class TestServiceValidator extends ServiceValidator<TestDTO> {
        TestServiceValidator(Translator translator) {
            super(translator);
        }

        @Override
        public void validate(TestDTO object, com.ia.core.service.exception.ServiceException exception) {
