...
import com.ia.core.security.service.config.SecurityConfigurationProvider;
import com.ia.core.security.service.config.SecurityProperties;
...
public abstract class CoreRestSecurityConfig
     ...
     *
     */
    public CoreRestSecurityConfig(SecurityConfigurationProvider configurationProvider) {
    ...
    protected AuthenticationEntryPoint authenticationEntryPoint() {
        ...
        CoreJwtAuthenticationEntryPoint authenticationEntryPoint = new CoreJwtAuthenticationEntryPoint();
     ...
     */
    protected OncePerRequestAuthenticationFilter authenticationFilter(UserDetailsService userDetailsService) {
        log.info(LOG_INICIALIZANDO_AUTH_FILTER);
        CoreJwtAuthenticationFilter authenticationFilter = new CoreJwtAuthenticationFilter(userDetailsService);
    ...
    CoreSecurityAuthorizationManager authorizationManager() {
        ...
        CoreAuthorizationManager authorizationManager = new CoreAuthorizationManager() {
            @Override
    ...
    PasswordEncoder passwordEncoder() {
        ...
        RestPasswordEncoder passwordEncoder = new RestPasswordEncoder();
    ...
    @Order(1)
    SecurityFilterChain securityLoginFilterChain(HttpSecurity http,
    ...
    @Order(2)
    SecurityFilterChain securityPublicFilterChain(HttpSecurity http,
                                                  @Value(PUBLIC_ENDPOINT_PATTERN) String authorizationPattern)
        throws Exception {
        String swaggerUiPath = getSecurityConfigurationProvider().getSecurityProperties().getSwagger().getUiPath();
        String swaggerApiDocsPath = getSecurityConfigurationProvider().getSecurityProperties().getSwagger().getApiDocsPath();
        String swaggerApiDocsYaml = getSecurityConfigurationProvider().getSecurityProperties().getSwagger().getApiDocsYaml();
    ...
    @Order(4)
    SecurityFilterChain securityMcpFilterChain(HttpSecurity http,
    ...
    UserDetailsService userDetailsService(UserRepository repository) {
        ...
        CoreUserDetailsService userDetailsService = new CoreUserDetailsService(repository);
    ...
    OpenAPI customOpenAPI() {
        SecurityProperties.SwaggerConfig swagger = getSecurityConfigurationProvider().getSecurityProperties().getSwagger();
        SecurityScheme securityScheme = new SecurityScheme()
            .name(swagger.getSecuritySchemeName()).type(SecurityScheme.Type.HTTP)
            .scheme(swagger.getSecurityScheme()).bearerFormat(swagger.getSecurityBearerFormat());

        return new OpenAPI()
            .info(new Info().title(swagger.getOpenapiTitle()).version(swagger.getOpenapiVersion())
                .description(swagger.getOpenapiDescription()))
            .addSecurityItem(new SecurityRequirement()
                .addList(swagger.getSecuritySchemeName()))
            .components(new io.swagger.v3.oas.models.Components()
                .addSecuritySchemes(swagger.getSecuritySchemeName(), securityScheme));
    }
